...
stock void GetSpawnBoxOrigins(int iBoxIndex, float vBoxOrigin[3], float vMins[3], float vMaxs[3])
{
	...
	for (int i; i < 3; i++)
	{
		vBoxOrigins[i] = flArray[(i * 3) + 0];
		vBoxOrigins[i] = flArray[(i * 3) + 1];
		vBoxOrigins[i] = flArray[(i * 3) + 2];
	}
...
stock int AddSpawnBoxOrigins(float vBoxOrigin[3], float vMins[3], float vMaxs[3])
{
	...
	for (int i; i < 3; i++)
	{
		flArray[(i * 3) + 0] = vBoxOrigins[i];
		flArray[(i * 3) + 1] = vBoxOrigins[i];
		flArray[(i * 3) + 2] = vBoxOrigins[i];
	}
