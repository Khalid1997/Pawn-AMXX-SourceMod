...
ConnectToGlobalDB()
{
	...
	{
		if (StrContains(szError, "Access denied for user", true) != -1)
...
{
	if (g_global_ValidedMap && g_global_IntegratedButtons && !g_global_VersionBlocked && !g_bGlobalBeta && g_global_EntityCheck && !g_global_SelfBuiltButtons && g_hDbGlobal != INVALID_HANDLE && g_bEnforcer && g_global_ValidFileSize && !g_bAutoTimer)
		return true;
	return false;
}
...
{
	if (g_hDbGlobal == INVALID_HANDLE || g_bGlobalBeta) return;
	decl String:szQuery[256];
		...
		if (g_global_Disabled)
		{
			...
			if(!StrEqual(g_szMapPrefix[0],"kz") && !StrEqual(g_szMapPrefix[0],"xc") && !StrEqual(g_szMapPrefix[0],"bkz")  && !StrEqual(g_szMapPrefix[0],"kzpro"))
			{
				...
				if (g_global_VersionBlocked)
				{
					...
					if (g_bGlobalBeta)
					{
						PrintToChat(client, "[%cKZ%c] %cGlobal Records disabled. Reason: Map is under construction. (alpha/beta version)!",MOSSGREEN,WHITE,RED);
						return Plugin_Handled;
					}
					else
