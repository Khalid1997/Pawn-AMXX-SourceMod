...
{	
	...
	//NEW GLOBAL RECORD 
	if (IsGlobalMap() && hax < 0.05 && !g_global_SelfBuiltButtons && g_global_Enforcer[client] && !g_bPositionRestored[client] && !g_global_DoubleDuck[client] && !g_bFlagged[client] && !g_global_AutoTimerOnStart[client] && !g_global_AutoBhopDetected[client] && g_fFinalTime[client] > 5.0)
	{	
	...
	if (IsGlobalMap() && !g_global_DoubleDuck[client] && g_global_Enforcer[client] && !g_global_SelfBuiltButtons && !g_bPositionRestored[client] && hax < 0.05 && !g_bFlagged[client] && !g_global_AutoTimerOnStart[client] && g_global_Access && !g_global_AutoBhopDetected[client] && g_fFinalTime[client] > 5.0)
	{			
	...
	else
	{
		if(StrEqual(g_szMapPrefix[0],"kz") || StrEqual(g_szMapPrefix[0],"xc") || StrEqual(g_szMapPrefix[0],"bkz")  || StrEqual(g_szMapPrefix[0],"kzpro"))
		{
			...
			if (g_bGlobalBeta)
				PrintToChat(client, "[%cKZ%c] %cGlobal Records disabled. Reason:  Map is under construction. (alpha/beta version)%c",MOSSGREEN, WHITE, RED, WHITE);
			else
			if (hax >= 0.05)
				PrintToChat(client, "[%cKZ%c] %cGlobal Records disabled. Reason: Server-sided bunnyhop script detected.%c",MOSSGREEN, WHITE, RED, WHITE);
			else
			if (g_bDoubleDuckCvar)
