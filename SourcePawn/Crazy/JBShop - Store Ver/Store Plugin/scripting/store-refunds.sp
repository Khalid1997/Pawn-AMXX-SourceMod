...
public void OnRemoveUserItemComplete(int accountId, int itemId, any data)
{
	...
	int iPrice;
	int credits = RoundToZero( ( iPrice = Store_GetItemPrice(itemId) ) * g_refundPricePercentage);
	...
	Store_GetItemName(itemId, szItemName, sizeof szItemName);
	char szLogReason[MAX_LOG_REASON_LENGTH];
	FormatEx(szLogReason, sizeof szLogReason, "Refund on (%0.2f * %d): %s", g_refundPricePercentage, iPrice, szItemName);
	Store_GiveCredits(accountId, credits, OnGiveCreditsComplete, hPack, szLogReason);
}
