...
public void OnPluginStart()
{
	...
	ConVar_MainMenuEnabled = CreateConVar("store_menu_enabled", "1");
...
void SortMainMenuItems()
{
	...
	for (int x = 0; x < g_menuItemCount; x++)
	{
		...
		{
			//PrintToServer("%d %s -  %d %s", g_menuItems[x][MenuItemOrder], g_menuItems[x][MenuItemDisplayName], g_menuItems[y][MenuItemOrder], g_menuItems[y][MenuItemDisplayName]);
			
			if (g_menuItems[x][MenuItemOrder] < g_menuItems[y][MenuItemOrder])
			{
				//PrintToServer("True On That");
...
{
	if(!ConVar_MainMenuEnabled.BoolValue)
	{
		PrintToChat(client, "* Store is disabled right now.");
		return;
	}
