...
public void DropGetCreditsCallback(int credits, any hPack)
{
	...
	if (credits >= needed)
	{
		Store_RemoveCredits(GetSteamAccountID(client), needed, DropGiveCreditsCallback, GetClientOfUserId(client), "Dropped Credits");
	}
	else
	{
...
void GiftCredits(int from, int to, int amount)
{
	...
	FormatEx(szLogReason, sizeof szLogReason, "Gift to %d", GetSteamAccountID(to));
	Store_RemoveCredits(GetSteamAccountID(from), amount, TakeCreditsCallback, hPack, szLogReason);
}
...
public void TakeCreditsCallback(int accountId, int credits, bool bIsNegative, any hPack)
{
	...
	char szLogReason[MAX_LOG_REASON_LENGTH];
	FormatEx(szLogReason, sizeof szLogReason, "Gift from %d", GetSteamAccountID(from));
	Store_GiveCredits(GetSteamAccountID(to), credits, GiveCreditsCallback, hPack, szLogReason);
}
...
public void OnStartTouch(int present, int client)
{
	...
	if (StrEqual(values[0],"credits"))
	{
		...
		char szLogReason[MAX_LOG_REASON_LENGTH];
		FormatEx(szLogReason, sizeof szLogReason, "Preset from %d", GetSteamAccountID(g_spawnedPresents[present][Present_Owner]));
		Store_GiveCredits(GetSteamAccountID(client), StringToInt(values[1]), PickupGiveCallback_Credits, hPack, szLogReason);
	}
