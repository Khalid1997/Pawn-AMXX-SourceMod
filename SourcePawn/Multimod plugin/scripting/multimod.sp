...
public void OnAllPluginsLoaded()
{
	...
	MultiMod_Settings_Create(MM_SETTING_RANDOM_FIRST_MOD, "1", true, true, true);
...
bool IsPluginProperlyLoaded()
{
	...
	if (g_iLoadStatus == LS_Reload)
	{
		...
		if (!g_iModsCount)
		{
			SetFailState("[MultiMod] Could not find any MODs to start. ERRCDE: 3");
			return false;
		}
	...
	if (g_iLoadStatus == LS_Loaded)
	{
		if (!g_iModsCount)
		{
			SetFailState("[MultiMod] Could not find any MODs to start. ERRCDE: 1");
			return false;
		}
	...
	if (g_iLoadStatus == LS_None)
	{
		if (!g_iModsCount)
		{
			SetFailState("[MultiMod] Could not find any MODs to start. ERRCDE: 2");
			return false;
		}
...
void ReadFiles(bool bCheckCurrModID = false)
{
	...
	if (!FileExists(szMultiModINI))
	{
		...
		{
			CreateDirectory(g_szMultiModFolderPath, 511);
		}
	...
	while ( ReadFileLine(f, szLine, sizeof(szLine) ) )
	{
		...
		switch (iPhase)
		{
			case 1:
			{
				...
				int iSplit = ExplodeString(szLine, ":", szModStuff, sizeof(szModStuff), sizeof(szModStuff[]), true);
...
public int Native_BuildPath(Handle hPlugin, int iArgs)
{
	...
	int iSize = GetNativeCell(4);
	char[] szFormatOutput = new char[iSize];
	FormatNativeString(0, 5, 6, iSize, _, szFormatOutput, _);
	...
	switch (iPath)
	{
		...
		{
			iLen = strcopy(szFormatOutput, iSize, g_szMultiModFolderPath);
		}
		
		case MultiModPath_Mods:
		{
			iLen = strcopy(szFormatOutput, iSize, g_szMultiModFolderPath_Mods);
		}
		...
		case MultiModPath_ModFolder:
		{
			...
			iLen = FormatEx(szFormatOutput, iSize, "%s/%s", g_szMultiModFolderPath_Mods, szModProp);
		}
	}

	SetNativeString(3, szFormatOutput, iSize);
	return iLen;
}
