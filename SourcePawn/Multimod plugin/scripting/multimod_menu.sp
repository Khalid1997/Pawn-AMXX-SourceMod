...
public void MultiMod_Vote_OnVoteStart(MultiModVote iAllVoteBit, MultiModVote iCurrentVote, MultiModVoteType iVoteType, bool bForceChange)
{
	g_bVotingStarted = true;
}

public void MultiMod_Vote_OnVoteFinished(MultiModVote iAllVoteBit, MultiModVote iVoteProgressBit)
{
	if(iAllVoteBit == iVoteProgressBit)
	{
		g_bVotingStarted = false;
	}
}
...
void BuildMultiModMenu()
{
	...
	AddMenuItem(g_hMultiModMenu_StartVote, g_szVoteMenuItemsInfo[VoteMenuItem_NormalVote], "Start Mod Vote (Doesn't Respect NextMod)", ITEMDRAW_DEFAULT);
...
public int MultiModMenu_Vote_MenuHandler(Handle menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		...
		case MenuAction_DisplayItem:
		{
			...
			if(iItemIndex == VoteMenuItem_Force)
			{
...
{
	if(GetUserFlagBits(client) & iFlagBit || GetUserFlagBits(client) & ADMFLAG_ROOT)
	{
		return true;
	}
