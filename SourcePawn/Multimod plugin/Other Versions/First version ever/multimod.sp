...
public Action:Cmd_Say(client, const String:szCommand[], iArgCount)
{
	...
	{
		PrintToChat(client, "[MultiMod] Current MOD: %s", g_szCurrModProps[MP_NAME]);
	}
	
	else if(StrEqual(szMMCmd[1], "nextmod", false))
	{
		PrintToChat(client, "[MultiMod] Next MOD: %s", g_iNextModId == -1 ? "Not chosen yet." : g_szNextModProps[MP_NAME]);
	}
}
...
ReadFiles(iCheckCurrModID = 0)
{
	...
	new String:szSourceModPath[60];
	
	BuildPath(Path_SM, szSourceModPath, sizeof(szSourceModPath), "");
	 
	FormatEx(szMultiModINI, sizeof(szMultiModINI), "%s/configs/multimod.ini", szSourceModPath)
	
	if(!FileExists(szMultiModINI))
	{
		new Handle:f = OpenFile(szMultiModINI, "w");
		WriteFileLine(f, 
		 "; --------------------------------------------------------------------------------------------------\
		...
		FormatEx(szMultiModINI, sizeof(szMultiModINI), "cfg/multimod", szSourceModPath);
		CreateDirectory(szMultiModINI, 0);
	...
	FormatEx(szMultiModINI, sizeof(szMultiModINI), g_szCurrModProps[MP_CFG]);
	
	if(szMultiModINI[0])
	{
		if(StrContains(szMultiModINI, ".cfg", false))
		{
			ReplaceString(szMultiModINI, sizeof(szMultiModINI), ".cfg", "", false);
		}
		
		//AutoExecConfig(true, szMultiModINI, "multimod");
		ServerCommand("exec cfg/multimod/%s", szMultiModINI);
	}
}
...
ReadFiles2(String:szFile[], bool:bCheck = false)
{
	...
	while(!IsEndOfFile(f))
	{
		...
		switch(iPhase)
		{
			case 1:
			{
				...
				PrintToServer("[MultiMod] MOD Loaded: [%s] - [%s] - [%s] - [%s]", szModStuff[MP_NAME], szModStuff[MP_PLUGIN], szModStuff[MP_MAP], szModStuff[MP_CFG]);
	...
	PrintToServer("[MultiMod] Total MOD count: %d MOD(s)", g_iModsCount);
...
ChangePlugins(iDontMoveNextMod = 0)
{
	...
	new String:szOldPath[60], String:szNewPath[60], String:szModPluginFolder[60];
	...
	else
	{
		...
		if(!DirExists(szModPluginFolder))
		{
			FormatEx(szModPluginFolder, sizeof szModPluginFolder, "disabled");
		}
	}
...
public Native_GetModProps(Handle:hPlugin, iArgs)
{
	...
	GetArrayString(gModsArrays[GetNativeCell(2)], iIndex, szData, iSize);
