				...
				if (found_func && ReadFuncString(buffer, func_tag, funcname) && IsValidString(funcname))
				{
					if (search[0])
					{
						if (StrEqual(funcname, search))
						{
							...
							new String:include[60];
							GetIncludeName(include, sizeof include, filepath);
							WriteFileLine(g_FileSourcemod, "Include :%s", include);
...
GetIncludeName(String:include[], size, String:filepath[])
{
	strcopy(include, size, filepath);
	ReplaceString(include, size, PATH_INCLUDE, "", false);
	ReplaceString(include, size, "/", "", false);
}
