...
public void SQLCallback_CheckMatch(Handle hSql, Handle hResult, char[] szError, any data)
{
	...
	PrintToServer("MatchId: %d", iMatchId);
...
{
	char szAuthId[MAX_AUTHID_LENGTH]; GetClientAuthId(client, AuthId_SteamID64, szAuthId, sizeof szAuthId);
...
void CheckIfClientIsAllowed(client)
{
	...
	char szQuery[512];
	FormatEx(szQuery, sizeof szQuery, "SELECT `%s` FROM `%s` WHERE `%s` = %d AND `%s` = '%s'", 
	...
	PrintDebug(szQuery);
	SQL_TQuery_Custom(g_hSql, SQLCallback_CheckClient, client, _, "Select Player Query:", szQuery);
}
