...
public Event_StartRound()
{
	...
	if(winner)
	{
		...
		static iMatches
		if( ++ g_iMatches >= ( iMatches = get_pcvar_num(g_pMatches) ) )
		{
			switch(callfunc_begin("start_vote", "multimod.amxx"))
			{
				...
				{
					callfunc_end()
					client_print(0, print_chat, "[SJ] %d matches are enough! Starting the mod vote", iMatches)
					return;
				}
			}
		}
