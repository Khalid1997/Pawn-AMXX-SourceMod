...
public message_SendAudio(msg_id, msg_dest, id)
{
	...
	if(equal(szSoundCode, "%!MRAD_terwin"))
	{
		if(AllowDraw())
	...
	else if(equal(szSoundCode, "%!MRAD_ctwin"))
	{
		new sound[60]
...
{
	if( msg_dest == MSG_ALL && get_msg_arg_int(1) == print_center ) 
	{
		static szCode[50]
		...
		{
			formatex(szWinMsg, charsmax(szWinMsg), "%L", OFFICIAL_LANG, "ESCAPE_DRAW")
			set_msg_arg_string(2, szWinMsg)
		}
		
		else if(equal(szCode, "#CTs_Win"))
		{
			formatex(szWinMsg, charsmax(szWinMsg), "%L", OFFICIAL_LANG, "ESCAPE_SUCCESS")
			set_msg_arg_string(2, szWinMsg);
		}
		
		else if(equal(szCode, "#Terrorists_Win"))
		{
			formatex(szWinMsg, charsmax(szWinMsg), "%L", OFFICIAL_LANG, "ESCAPE_FAIL")
			set_msg_arg_string(2, szWinMsg);
		}
	}
}
...
public check_win_con()
{
	...
	{
		server_print("SLAY CT");
		for(new i = 1; i <= g_MaxPlayers; i++)
		{
			if(is_user_alive(i) && get_user_team(i) == 2)
			{
				server_print("SLAY %d", i);
				user_silentkill(i);
			}
		}
	...
	if(get_player_num(TEAM_ALL, AL_BOTH) <= 1)
	{
		g_gamestart = 0
	}
}
