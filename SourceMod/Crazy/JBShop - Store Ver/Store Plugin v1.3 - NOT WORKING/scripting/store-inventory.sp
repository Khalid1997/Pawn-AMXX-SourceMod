...
public int Native_Item_UseItem(Handle hPlugin, int iArgs)
{
	...
	if (callbackValue != Store_DoNothing)
	{
		...
		if (callbackValue == Store_EquipItem)
		{
			...
			{
				Store_SetItemEquippedState(auth, id, Store_GetClientLoadout(client), true, EquipItemCallback_Custom, GetClientUserId(client));
			}
		}
		else if (callbackValue == Store_UnequipItem)
		{
			...
			{
				Store_SetItemEquippedState(auth, id, Store_GetClientLoadout(client), false, EquipItemCallback_Custom, GetClientUserId(client));
			}
		}
