...
char sQuery_GetUserItems[] = "SELECT item_id, EXISTS(SELECT * FROM %s_users_items_loadouts WHERE %s_items_loadouts.item_id = %s_users_items.id AND %s_items_loadouts.loadout_id = %d) AS equipped, COUNT(*) AS count FROM %s_users_items INNER JOIN %s_users ON %s_users.id = %s_users_items.user_id INNER JOIN %s_items ON %s_items.id = %s_users_items.item_id WHERE %s_users.auth = %d AND ((%s_users_items.acquire_date IS NULL OR %s_items.expiry_time IS NULL OR %s_items.expiry_time = 0) OR (%s_users_items.acquire_date IS NOT NULL AND %s_items.expiry_time IS NOT NULL AND %s_items.expiry_time <> 0 AND DATE_ADD(%s_users_items.acquire_date, INTERVAL %s_items.expiry_time SECOND) > NOW()))";
...
char sQuery_GiveCredits_Custom[] = "UPDATE %s_users SET credits = IF(credits + %d > %d, %d, credits + %d) WHERE auth = %d";
...
char sQuery_GiveCreditsToUsers_Custom[] = "UPDATE %s_users SET credits = IF(credits + %d > %d, %d, credits + %d) WHERE auth IN (";
char sQuery_GiveCreditsToUsers_End[] = "%s)";
...
char sQuery_CreateTable_UsersLoadouts[] = "CREATE TABLE IF NOT EXISTS `%s_users_loadouts` ( \
...
char sQuery_CreateTable_LogTable[] = "CREATE TABLE IF NOT EXISTS %s_users_credits_log ( accountID INT, credits INT, updated_credits INT, reason VARCHAR(256), time_of_purchase DATETIME DEFAULT CURRENT_TIMESTAMP())";
...
char g_szLastPriorityString[256] = "High";
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("Store_GetClientUserID_Ex", Native_GetClientUserID_Ex);
...
{
	int index = GetItemIndex(iItem);
	Handle hArray = GetArrayCell(hArray_Items, index);
	
	int iPrice = GetArrayCell(hArray, 7);
...
public Action Command_GiveCredits(int client, int args)
{
	...
	FormatEx(szLogReason, sizeof szLogReason, "Admin %N Give", client);
	
	Store_GiveCreditsToUsers(accountIds, count, iMoney, szLogReason);
	return Plugin_Handled;
}
...
public Action Command_RemoveCredits(int client, int args)
{
	...
	char szLogReason[MAX_LOG_REASON_LENGTH];
	FormatEx(szLogReason, sizeof szLogReason, "Admin %N Remove", client);
	for (int i = 0; i < target_count; i++)
	{
		if (!IsClientInGame(target_list[i]) || IsFakeClient(target_list[i]))
		{
		...
		Store_RemoveCredits(GetSteamAccountID(target_list[i]), iMoney, OnRemoveCreditsCallback, GetClientUserId(client), szLogReason);
	}
...
bool GetItems(int client = 0, Handle filter = null, Store_GetItemsCallback callback = INVALID_FUNCTION, Handle plugin = null, bool loadFromCache = true, const char[] sPriority = "", any data = 0)
{
	...
	else
	{
		...
		char sQuery[MAX_QUERY_SIZE];
		Format(sQuery, sizeof(sQuery), sQuery_GetItems, STORE_DATABASE_PREFIX, g_szLastPriorityString);
...
public void OnGetCreditsForItemBuy(int credits, any hPack)
{
	...
	PrintToChatAll("Attempt Buy: %d %d", credits, iItemPrice);
	...
	char szItemName[50];
	GetArrayString(hArray, 3, szItemName, sizeof szItemName);
	
	char szLogReason[MAX_LOG_REASON_LENGTH];
	FormatEx(szLogReason, sizeof szLogReason, "ShopBuy: %s", szItemName);

	RemoveCredits(accountId, iItemPrice, OnBuyItemGiveItem, _, hPack, szLogReason);
}
...
void GiveCredits(int accountId, int credits, Store_GiveCreditsCallback callback, Handle plugin = null, any data = 0, char[] szLogReason)
{
	...
	char sQuery[MAX_QUERY_SIZE];
	Format(sQuery, sizeof(sQuery), sQuery_GiveCredits_Custom, STORE_DATABASE_PREFIX, credits, MAX_CREDITS, MAX_CREDITS, credits, accountId);
...
void RemoveCredits(int accountId, int credits, Store_RemoveCreditsCallback callback, Handle plugin = null, any data = 0, char[] szReason)
{
	if (!IsDatabaseConnected())
	{
		return;
...
public void SQLCall_RemoveCredits(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	char szReason[MAX_LOG_REASON_LENGTH];
	ReadPackString(data, szReason, MAX_LOG_REASON_LENGTH);
	...
	LogCreditsQuery(accountId, -credits, szReason);
}
...
void GiveCreditsToUsers(int[] accountIds, int accountIdsLength, int credits, char[] szLogReason)
{
	...
	char sQuery[MAX_QUERY_SIZE];
	Format(sQuery, sizeof(sQuery), sQuery_GiveCreditsToUsers_Custom, STORE_DATABASE_PREFIX, credits, MAX_CREDITS, MAX_CREDITS, credits);
...
void GiveDifferentCreditsToUsers(int[] accountIds, int accountIdsLength, int[] credits, char[] szReason)
{
	if (!IsDatabaseConnected())
	{
		return;
...
void RemoveDifferentCreditsFromUsers(int[] accountIds, int accountIdsLength, int[] credits, char[] szLogReason)
{
	...
	for (int i = 0; i < accountIdsLength; i++)
	{
		Format(sQuery, sizeof(sQuery), sQuery_GiveDifferentCreditsToUsers_accountIdsLength_Custom, sQuery, accountIds[i], credits[i], MAX_CREDITS, MAX_CREDITS, credits[i]);
	}
...
void ConnectSQL()
{
	...
	bool bConfigFound = SQL_CheckConfig(sBuffer);
	...
	SQLCall_ConnectToDatabase(INVALID_HANDLE, g_hSQL, "", INVALID_HANDLE);
...
public void SQLCall_ConnectToDatabase(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	Format(sQuery, sizeof(sQuery), sQuery_CreateTable_UsersLoadouts, STORE_DATABASE_PREFIX);
	SQL_AddQuery(trans, sQuery);
	
	Format(sQuery, sizeof(sQuery), sQuery_CreateTable_LogTable, STORE_DATABASE_PREFIX);
	SQL_AddQuery(trans, sQuery);
...
public int Native_GetClientUserID(Handle plugin, int numParams)
{
	...
	{
		char sQuery[MAX_QUERY_SIZES];
		FormatEx(sQuery, sizeof(sQuery), sQuery_GetClientUserID, STORE_DATABASE_PREFIX, GetSteamAccountID(client));
...
public int Native_GetClientUserID_Ex(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
...
public int Native_GiveCredits(Handle plugin, int numParams)
{
	...
	GiveCredits(GetNativeCell(1), GetNativeCell(2), view_as<Store_GiveCreditsCallback>(GetNativeFunction(3)), plugin, view_as<any>(GetNativeCell(4)), szLogReason);
}
...
public int Native_RemoveCredits(Handle plugin, int numParams)
{
	...
	RemoveCredits(GetNativeCell(1), GetNativeCell(2), view_as<Store_RemoveCreditsCallback>(GetNativeFunction(3)), plugin, view_as<any>(GetNativeCell(4)), szLogReason);
}
