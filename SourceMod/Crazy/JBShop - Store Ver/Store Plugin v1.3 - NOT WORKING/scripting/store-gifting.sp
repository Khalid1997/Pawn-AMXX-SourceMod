...
public void DropGetCreditsCallback(int credits, any hPack)
{
	...
	if (credits >= needed)
	{
		Store_RemoveCredits(GetSteamAccountID(client), needed, DropGiveCreditsCallback, GetClientOfUserId(client), "Present/Gift Drop");
	}
	else
	{
...
void GiftCredits(int from, int to, int amount)
{
	...
	FormatEx(szReason, sizeof szReason, "Gift to Player %N", to);

	Store_RemoveCredits(GetSteamAccountID(from), amount, TakeCreditsCallback, hPack, szReason);
}
...
public void TakeCreditsCallback(int accountId, int credits, bool bIsNegative, any hPack)
{
	...
	char szReason[MAX_LOG_REASON_LENGTH];
	FormatEx(szReason, sizeof szReason, "Gift from Player %N", from);

	Store_GiveCredits(GetSteamAccountID(to), credits, GiveCreditsCallback, hPack, szReason);
}
...
public void OnStartTouch(int present, int client)
{
	...
	{
		char szReason[MAX_LOG_REASON_LENGTH];
		FormatEx(szReason, sizeof szReason, "Present/Gift unbox from %N", g_spawnedPresents[present][Present_Owner]);
		Store_GiveCredits(GetSteamAccountID(client), StringToInt(values[1]), PickupGiveCallback_Credits, hPack, szReason);
	}
