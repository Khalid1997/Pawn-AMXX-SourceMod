...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	g_Array_CustomItems = CreateArray(1);
...
public void GetCategoriesCallback(int[] ids, int count, any data)
{
	...
	Menu hMenu = CreateMenu(ShopMenuSelectHandle);
	SetMenuTitle(hMenu, "%T%T\n \n", "Store Menu Title", client, "Store Menu Shop Menu", client);
	...
	for(int i; i < iSize; i++)
	{
		...
		hMenu.AddItem(szInfo, szDisplayName);
	}
...
public void GetItemsCallback(int[] ids, int count, any hPack)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		FormatEx(sPrice, sizeof sPrice, "%d", Store_Custom_GetItemPrice(client, ids[i]));
