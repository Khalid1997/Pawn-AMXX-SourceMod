...
#define USE_STORE_VERSION VERSION_12
...
bool g_bFPVM_Interface = false;
bool g_bFDownloader = false;
bool g_bDaysAPI;
...
#define MAX_PARAMS 5
#define MAX_UPGRADES_PLUGIN (9 + 1)
...
char g_szLightSaberModelsFiles[][] = {
	"materials/models/weapons/eminem/master_word/*",
	"models/weapons/eminem/master_sword/*"
};

char g_szLightSaberViewModel[] = "models/weapons/eminem/master_sword/v_master_sword.mdl";
char g_szLightSaberWorldModel[] = "models/weapons/eminem/master_sword/w_master_sword.mdl";
int g_iLightSaberViewModel, g_iLightSaberWorldModel;
...
{
	if(StrEqual(szName, "fpvm_interface"))
	{
		g_bFPVM_Interface = true;
	}
	
	else if(StrEqual(szName, "fdownloader"))
	{
		g_bFDownloader = true;
	}
...
public void OnLibraryRemoved(const char[] szName)
{
	if(StrEqual(szName, "fpvm_interface"))
	{
		g_bFPVM_Interface = false;
	}
	
	else if(StrEqual(szName, "fdownloader"))
	{
		g_bFDownloader = false;
	}
...
public void OnPluginStart()
{
	...
	{
		#if USE_STORE_VERSION == VERSION_13
		Store_OnDatabaseInitialized(INVALID_HANDLE);
		#else
...
{
	g_bFDownloader = LibraryExists("fdownloader");
	g_bDaysAPI = LibraryExists("daysapi");
	g_bFPVM_Interface  = LibraryExists("fpvm_interface");
...
public void OnMapStart()
{
	...
	if(g_bFDownloader && g_bFPVM_Interface)
	{
		FDownloader_AddPathsEx(g_szLightSaberModelsFiles, sizeof g_szLightSaberModelsFiles);
		g_iLightSaberViewModel = PrecacheModel(g_szLightSaberViewModel, true);
		g_iLightSaberWorldModel = PrecacheModel(g_szLightSaberWorldModel, true);
	}
...
public void OnClientPutInServer(int client)
{
	...
	SetArrayValue(g_iClientCurrentItemUpgrade[client], sizeof g_iClientCurrentItemUpgrade[], ItemUpgrade_Base);
...
#if USE_STORE_VERSION == VERSION_13
public void StoreCallback_GetUserItems(int accountId, int[] useritems, bool[] equipped, int[] useritemCount, int count, int loadoutId, any client)
#else
...
void DoBuyClientUpgrade(int client, int iItem, int iUpgrade)
{
	...
	//Callback_BuyItemUpgrade(client, 99999, hPack);
}
...
{
	//PrintToChatAll("Buy Item Declined %d %d %d", client, iItem, bPrintReason);
	if (!g_bCanBuy)
	{
