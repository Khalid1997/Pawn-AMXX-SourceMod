...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("Store_Custom_UseItem", Native_Item_UseItem);
	
	// Edit
	CreateNative("Store_Custom_RemoveItemType", Native_RemoveItemType);
		
	RegPluginLibrary("store-inventory");	
	return APLRes_Success;
}

public int Native_Item_UseItem(Handle hPlugin, int iArgs)
{
	...
	if (!Store_IsItemTypeRegistered(type))
	{
	...
	if (callbackValue != Store_DoNothing)
	{
		...
		if (callbackValue == Store_EquipItem)
		{
			...
			{
				Store_SetItemEquippedState(auth, id, Store_GetClientCurrentLoadout(client), true, EquipItemCallback_Custom, GetClientUserId(client));
			}
		}
		else if (callbackValue == Store_UnequipItem)
		{
			...
			{
				Store_SetItemEquippedState(auth, id, Store_GetClientCurrentLoadout(client), false, EquipItemCallback_Custom, GetClientUserId(client));
			}
		}
		else if (callbackValue == Store_DeleteItem)
		{
			Store_RemoveUserItem(auth, id, UseItemCallback_Custom, GetClientUserId(client));
		}
	}
}
public void EquipItemCallback_Custom(int accountId, int itemId, int loadoutId, any data)
{

}

public void UseItemCallback_Custom(int accountId, int itemId, any data)
{
...
void RegisterItemType(const char[] type, Handle plugin, Store_ItemUseCallback useCallback, Store_ItemGetAttributesCallback attrsCallback = INVALID_FUNCTION)
{
	...
	// Edit
	if(GetTrieValue(g_itemTypeNameIndex, type, itemTypeIndex))
	{
		delete view_as<DataPack>(GetArrayCell(g_itemTypes, itemTypeIndex));
		SetArrayCell(g_itemTypes, itemTypeIndex, itemType);
		SetTrieValue(g_itemTypeNameIndex, type, itemTypeIndex);
	}
	
	else
	{
		itemTypeIndex = PushArrayCell(g_itemTypes, itemType);
		SetTrieValue(g_itemTypeNameIndex, type, itemTypeIndex);
	}
}

// Edit
void RemoveItemType(const char[] type)
{
	int itemTypeIndex;
	if(GetTrieValue(g_itemTypeNameIndex, type, itemTypeIndex))
	{
		...
		RemoveFromArray(g_itemTypes, itemTypeIndex);
		RemoveFromTrie(g_itemTypeNameIndex, type);
		
		Handle snapshot = CreateTrieSnapshot(g_itemTypeNameIndex);
		int iSize = TrieSnapshotLength(snapshot);
		
		char szKey[50];
		int iIndex;
		for(int i; i < iSize; i++)
		{
			GetTrieSnapshotKey(snapshot, i, szKey, sizeof szKey);
			
			if(GetTrieValue(g_itemTypeNameIndex, szKey, iIndex))
			{
				if(iIndex > itemTypeIndex)
				{
					SetTrieValue(g_itemTypeNameIndex, szKey, iIndex - 1);
				}
			}
		}
...
public int Native_RemoveItemType(Handle plugin, int numParams)
{
	char type[STORE_MAX_TYPE_LENGTH];
	GetNativeString(1, type, sizeof(type));
	RemoveItemType(type);
