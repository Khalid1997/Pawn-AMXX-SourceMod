...
// Edit added max_credits to sql query
#define MAX_CREDITS 20000
// Edit
...
char sQuery_GetItems[] = "SELECT id, priority, name, display_name, description, type, loadout_slot, price, category_id, attrs, LENGTH(attrs) AS attrs_len, is_buyable, is_tradeable, is_refundable, flags, enable_server_restriction FROM store_items %s";
...
char sQuery_GiveCredits_Custom[] = "UPDATE store_users SET credits = IF(credits + %d > %d, %d, credits + %d) WHERE auth = %d";
...
char sQuery_GiveCreditsToUsers_Custom[] = "UPDATE store_users SET credits = IF(credits + %d > %d, %d, credits + %d) WHERE auth IN (";
...
char sQuery_GiveDifferentCreditsToUsers[] = "UPDATE store_users SET credits = CASE auth";
//char sQuery_GiveDifferentCreditsToUsers_accountIdsLength[] = "%s WHEN %d THEN %d";
char sQuery_GiveDifferentCreditsToUsers_accountIdsLength_Custom[] = "%s WHEN %d THEN IF(credits + %d > %d, %d, credits + %d)";
...
// Edit
Handle g_hForward_GetItemPrice;
char g_szLastPriorityString[256];
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	// Edit
	CreateNative("Store_Custom_GetItemPrice", Native_Custom_GetItemPrice);
	g_hForward_GetItemPrice = CreateGlobalForward("Store_Custom_OnGetItemPrice", ET_Ignore, Param_Cell, Param_Cell, Param_CellByRef);
	
	CreateNative("Store_Custom_IsDatabaseLoaded", Native_Custom_IsDatabaseLoaded);
	CreateNative("Store_Custom_GetSQLHandle", Native_Custom_GetSQLHandle);
	//Library
...
public int Native_Custom_IsDatabaseLoaded(Handle hPlugin, int iArgs)
{
	return view_as<int>(g_hSQL != null);
}

public int Native_Custom_GetSQLHandle(Handle hPlugin, int iArgs)
{
...
public void OnPluginStart()
{
	...
	if(g_bLate)
	{
		CheckServerProcessing(INVALID_HANDLE);
	}
}
...
public void SQLCall_RetrieveCategories(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	while (SQL_FetchRow(hndl))
	{
		...
		/*
		PrintToServer("Retrieved %d %d %s %s %s %d", g_categories[g_categoryCount][CategoryId],
			g_categories[g_categoryCount][CategoryPriority],
			g_categories[g_categoryCount][CategoryDisplayName],
			g_categories[g_categoryCount][CategoryDescription],
			 g_categories[g_categoryCount][CategoryRequirePlugin], g_categories[g_categoryCount][CategoryDisableServerRestriction]);
		*/
...
void GetItems(int client, Handle filter = null, Store_GetItemsCallback callback = INVALID_FUNCTION, Handle plugin = null, bool loadFromCache = true, const char[] sPriority, any data = 0)
{
	if (loadFromCache && g_itemCount != -1)
	{
		...
		{
			/* if( 
					(!categoryFilter || categoryId == g_items[item][ItemCategoryId]) && 
	...
	else
	{
		...
		// Fix incase items were reloaded
		if (sPriority[0])
		{
			strcopy(g_szLastPriorityString, sizeof g_szLastPriorityString, sPriority);
		}
		
		Format(sQuery, sizeof(sQuery), sQuery_GetItems, g_szLastPriorityString);
...
void GetLoadouts(Handle filter, Store_GetItemsCallback callback = INVALID_FUNCTION, Handle plugin = null, bool loadFromCache = true, any data = 0)
{
	...
	{
		//PrintToServer("** GetLoadouts Query");
...
public void SQLCall_GetLoadouts(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	while (SQL_FetchRow(hndl))
	{
		...
		//PrintToServer("Loaded LoadOut %d", g_loadouts[g_loadoutCount][LoadoutId]);
...
void GetUserItems(Handle filter, int accountId, int loadoutId, Store_GetUserItemsCallback callback, Handle plugin = null, any data = 0)
{
	...
	if (g_itemCount == -1)
	{
		//PrintToServer("GetUserItems: GetItems");
	...
	//PrintToServer("GetUserItems: Continue");
	
	char sQuery[MAX_QUERY_SIZES];
	//Format(sQuery, sizeof(sQuery), sQuery_GetUserItems, loadoutId, accountId);
	Format(sQuery, sizeof(sQuery), sQuery_GetUserItems, accountId);
...
void BuyItem(int accountId, int itemId, Store_BuyItemCallback callback, Handle plugin = null, any data = 0)
{
	...
	//OnGetCreditsForItemBuy(9999, hPack);
}
...
public void OnGetCreditsForItemBuy(int credits, any hPack)
{
	...
	int client = FindClientFromAccountId(accountId);
	int iItemPrice;
	if (!client)
	{
		iItemPrice = g_items[GetItemIndex(itemId)][ItemPrice];
	}
	
	else
	{
		iItemPrice = Custom_GetItemPrice(client, itemId);
	}
	...
	RemoveCreditsFromItemBuy(accountId, iItemPrice, OnBuyItemGiveItem, _, hPack);
}

// Edit
int FindClientFromAccountId(int accountId)
{
	...
	LogError("Could Not find client with account id %d", accountId);
	return 0;
}
...
void GiveCredits(int accountId, int credits, Store_GiveCreditsCallback callback, Handle plugin = null, any data = 0, char[] szLogReason)
{
	Handle hPack = CreateDataPack();
	WritePackCell(hPack, accountId);
	...
	// Edit
	WritePackString(hPack, szLogReason);
	
	char sQuery[MAX_QUERY_SIZES];
	Format(sQuery, sizeof(sQuery), sQuery_GiveCredits_Custom, credits, MAX_CREDITS, MAX_CREDITS, credits, accountId);
...
public void SQLCall_GiveCredits(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	ReadPackString(data, szLogReason, MAX_LOG_REASON_LENGTH);
	...
	// Edit
	LogCreditsQuery(accountId, credits, szLogReason);
}

void LogCreditsQuery(int accountId, int credits, char[] szLogReason)
{
	
	// CREATE TABLE IF NOT EXISTS store_users_credits_log ( accountID INT, credits INT, updated_credits INT, reason VARCHAR(256), time_of_purchase DATETIME DEFAULT CURRENT_TIMESTAMP())
	
	// INSERT INTO store_users_credits_log 
	//	( accountId, credits, updated_credits, reason ) VALUES (
	//	5,5,5,'test'); 
	char sQuery[MAX_QUERY_SIZES];
	FormatEx(sQuery, sizeof sQuery, "INSERT INTO store_users_credits_log \
		( accountId, credits, updated_credits, reason ) VALUES (\
		%d,  %d, (SELECT credits FROM store_users WHERE auth =  %d),  '%s'); ", accountId, credits, accountId, szLogReason);
	Store_Local_TQuery("GiveCredits_Logging", SQLCall_GiveCredits_Log, sQuery);
}

public void SQLCall_GiveCredits_Log(Handle owner, Handle hndl, const char[] error, any data)
{
	if (error[0])
	{
		LogError("SQL CreditsLog Query Error: %s", error);
	}
}

//Function - Remove credits from a client.
void RemoveCredits(int accountId, int credits, Store_RemoveCreditsCallback callback, Handle plugin = null, any data = 0, char[] szLogReason)
{
	Handle hPack = CreateDataPack();
	WritePackCell(hPack, accountId);
	WritePackCell(hPack, credits);
...
// Edit
void RemoveCreditsFromItemBuy(int accountId, int credits, Store_RemoveCreditsCallback callback, Handle plugin = null, any data = 0)
{	
	Handle hPack = CreateDataPack();
	WritePackCell(hPack, accountId);
	WritePackCell(hPack, credits);
	...
	Store_LogDebug("Native - RemoveCreditsFromItemBuy - accountId = %d, credits = %d", accountId, credits);
	
	bool bIsNegative;
	// Nobody cares if credits are going into negative.
	/*if (Store_GetCreditsEx(accountId) < credits)
	{
		bIsNegative = true;
		WritePackCell(hPack, bIsNegative);
		...
		char sQuery[MAX_QUERY_SIZES];
		Format(sQuery, sizeof(sQuery), sQuery_RemoveCredits_Negative, accountId);
		Store_Local_TQuery("RemoveCreditsFromItemBuy", SQLCall_RemoveCreditsFromItemBuy, sQuery, hPack);
	...
	char sQuery[MAX_QUERY_SIZES];
	Format(sQuery, sizeof(sQuery), sQuery_RemoveCredits, credits, accountId);
	Store_Local_TQuery("RemoveCreditsFromShopItemBuy", SQLCall_RemoveCreditsFromItemBuy, sQuery, hPack);
}

//Query - callback when a client has credits removed.
public void SQLCall_RemoveCreditsFromItemBuy(Handle owner, Handle hndl, const char[] error, any data)
{
	ResetPack(data);
	...
	Handle plugin = view_as<Handle>(ReadPackCell(data));
	DataPack originalData = ReadPackCell(data);
	int bIsNegative = view_as<bool>(ReadPackCell(data));
	
	CloseHandle(data);
	
	/* originalDataContents
	...
	* From RemoveCreditsFromItemBuy
	...
	data now is the up stuff.
	*/
	
	originalData.Reset();
	int iItemId = originalData.ReadCell();
	char szLogReason[MAX_LOG_REASON_LENGTH];
	
	FormatEx(szLogReason, sizeof szLogReason, "ShopBuy: %s", g_items[GetItemIndex(iItemId)][ItemName]);
...
void GiveCreditsToUsers(int[] accountIds, int accountIdsLength, int credits, char[] szLogReason)
{
	...
	Format(sQuery, sizeof(sQuery), sQuery_GiveCreditsToUsers_Custom, credits, MAX_CREDITS, MAX_CREDITS, credits);
	...
	ArrayList accountIdsArrayList = ArrayToArrayList(accountIds, accountIdsLength);
	hPack.WriteCell(accountIdsArrayList);
	hPack.WriteCell(credits);
	hPack.WriteString(szLogReason);
	
	Format(sQuery, sizeof(sQuery), sQuery_GiveCreditsToUsers_End, sQuery);
	Store_Local_TQuery("GiveCreditsToUsers", SQLCall_GiveCreditsToUsers, sQuery, hPack);
}

// Edit
ArrayList ArrayToArrayList(int[] array, int iSize)
{
...
public void SQLCall_GiveCreditsToUsers(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	ArrayList array = ReadPackCell(data);
	int credits = ReadPackCell(data);
	ReadPackString(data, szLogReason, sizeof szLogReason);
	...
	for (int i; i < iSize; i++)
	{
		LogCreditsQuery(array.Get(i), credits, szLogReason);
	}
...
void RemoveCreditsFromUsers(int[] accountIds, int accountIdsLength, int credits, char[] szLogReason)
{
	if (accountIdsLength == 0)
	{
		return;
	...
	Store_Local_TQuery("RemoveCreditsFromUsers", SQLCall_RemoveCreditsFromUsers, sQuery, hPack);
}
...
void GiveDifferentCreditsToUsers(int[] accountIds, int accountIdsLength, int[] credits, char[] szLogReason)
{
	if (accountIdsLength == 0)
	{
		return;
	...
	// Edit
	ArrayList array = ArrayToArrayList(accountIds, accountIdsLength);
	ArrayList creditsarray = ArrayToArrayList(credits, accountIdsLength);
	
	DataPack hPack = new DataPack();
	hPack.WriteCell(array);
	hPack.WriteCell(creditsarray);
	hPack.WriteString(szLogReason);
	
	for (int i = 0; i < accountIdsLength; i++)
	{
		// IF(credits + give > MAX, MAX, credits + give)
		Format(sQuery, sizeof(sQuery), sQuery_GiveDifferentCreditsToUsers_accountIdsLength_Custom, sQuery, 
			accountIds[i], credits[i], MAX_CREDITS, MAX_CREDITS, credits[i]);
	}
	...
	Format(sQuery, sizeof(sQuery), "%s)", sQuery);
	Store_Local_TQuery("GiveDifferentCreditsToUsers", SQLCall_GiveDifferentCreditsToUsers, sQuery, hPack);
}
...
public void SQLCall_GiveDifferentCreditsToUsers(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	// Edit
	ArrayList array = ReadPackCell(data);
	ArrayList creditsarray = ReadPackCell(data);
	...
	for (int i; i < iSize; i++)
	{
		LogCreditsQuery(array.Get(i), creditsarray.Get(i), szLogReason);
	}
...
void RemoveDifferentCreditsFromUsers(int[] accountIds, int accountIdsLength, int[] credits, char[] szLogReason)
{
	if (accountIdsLength == 0)
	{
		return;
	...
	Format(sQuery, sizeof(sQuery), "%s)", sQuery);
	Store_Local_TQuery("RemoveDifferentCreditsFromUsers", SQLCall_RemoveDifferentCreditsFromUsers, sQuery, hPack);
}
...
public void SQLCall_RemoveDifferentCreditsFromUsers(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	for (int i; i < iSize; i++)
	{
		LogCreditsQuery(array.Get(i), -1 * creditsarray.Get(i), szLogReason);
	}
...
void ConnectSQL()
{
	...
	for (int i; i < 5; i++)
	{
		g_hSQL = SQL_Connect(sBuffer, false, szError, sizeof szError);
	...
	if (g_hSQL == null)
	{
		SetFailState("Error: %s", szError);
		return;
	}
	
	DBResultSet result = SQL_Query(g_hSQL, "SET SESSION sql_mode = ''");
	if(result == null)
	{
		SetFailState("Error: Could not set sql_mode to appropriate value.");
		return;
	}
	...
	// Edit
	// This is a fix for the edit in Native_GetClientUserId errors
	// L 01/01/2018 - 14:33:08: [SM] Exception reported: Invalid database Handle 0 (error: 4)
	// L 01/01/2018 - 14:33:08: [SM] Blaming: store-backend.smx
	// L 01/01/2018 - 14:33:08: [SM] Call stack trace:
	// L 01/01/2018 - 14:33:08: [SM]   [0] SQL_TQuery
	// L 01/01/2018 - 14:33:09: [SM]   [1] Line 1964, C:\Users\Khalid\Desktop\Plugins\SourceMod\Crazy\JBShop - Store Ver\Store Plugin\scripting\store-backend.sp::OnClientPutInServer
...
void Store_Local_TQuery(const char[] sQueryName, SQLTCallback callback, const char[] sQuery, any data = 0, DBPriority prio = DBPrio_High)
{
...
// EDIT
int g_iUserId[MAXPLAYERS];
public void SQL_GetClientUserId_Callback(Handle hSql, Handle hQuery, char[] sError, int client)
{
	if (sError[0])
	{
	...
	{
		g_iUserId[client] = SQL_FetchInt(hQuery, 0);
	}
}
...
public void OnClientPutInServer(int client)
{
	...
	char sQuery[MAX_QUERY_SIZES];
	FormatEx(sQuery, sizeof(sQuery), sQuery_GetClientUserID, GetSteamAccountID(client));
	SQL_TQuery(g_hSQL, SQL_GetClientUserId_Callback, sQuery, client);
	/*
...
public int Native_GetClientUserID(Handle plugin, int numParams)
{
	...
	if (g_iUserId[client] == -1)
	{
		...
		SQL_TQuery(g_hSQL, SQL_GetClientUserId_Callback2, sQuery, dp);
	}
...
public void SQL_GetClientUserId_Callback2(Handle hSql, Handle hQuery, char[] sError, DataPack dp)
{
	if (sError[0])
	{
		Store_LogError("SQL Error on Native_GetClientUserID: %s", sError);
...
public int Native_Custom_GetItemPrice(Handle plugin, int numParams)
{
	return Custom_GetItemPrice(GetNativeCell(1), GetNativeCell(2));
}

int Custom_GetItemPrice(int client, int iItem)
{
	return DoGetItemPriceForward(client, iItem);
}

//Native - Gets an items price.
public int Native_GetItemPrice(Handle plugin, int numParams)
{
	return Custom_GetItemPrice(0, GetNativeCell(1));
	// Original
	//return g_items[GetItemIndex(GetNativeCell(1))][ItemPrice];
}

int DoGetItemPriceForward(int client, int iItem)
{
	int iPrice = g_items[GetItemIndex(iItem)][ItemPrice];
	int iRet;
	
	Call_StartForward(g_hForward_GetItemPrice);
...
public int Native_GetLoadouts(Handle plugin, int numParams)
{
	...
	//PrintToServer("GetLoadouts Native Start %s", GetPluginName(plugin));
	GetLoadouts(GetNativeCell(1), view_as<Store_GetItemsCallback>(GetNativeFunction(2)), plugin, GetNativeCell(3), data);
	//PrintToServer("GetLoadouts Native End");
}

// Edit
stock char[] GetPluginName(Handle hPlugin)
{
	char szBuffer[50];
	GetPluginFilename(hPlugin, szBuffer, sizeof szBuffer);
	return szBuffer;
}
...
{
	//LogError("%d %d", GetNativeCell(1), GetLoadoutIndex(GetNativeCell(1)));
...
public int Native_GiveCredits(Handle plugin, int numParams)
{
	...
	// Edit
	char szLogReason[MAX_LOG_REASON_LENGTH];
	GetNativeString(5, szLogReason, MAX_LOG_REASON_LENGTH);
	// finish edit
	
	GiveCredits(GetNativeCell(1), GetNativeCell(2), view_as<Store_GiveCreditsCallback>(GetNativeFunction(3)), plugin, data, szLogReason);
}
...
public int Native_GiveCreditsToUsers(Handle plugin, int numParams)
{
	...
	// Edit
	char szLogReason[MAX_LOG_REASON_LENGTH];
	GetNativeString(4, szLogReason, MAX_LOG_REASON_LENGTH);
	// Finish Edit
	
	GiveCreditsToUsers(accountIds, length, GetNativeCell(3), szLogReason);
}
...
public int Native_GiveDifferentCreditsToUsers(Handle plugin, int params)
{
	...
	GiveDifferentCreditsToUsers(accountIds, length, credits, szLogReason);
}
...
public int Native_RemoveCredits(Handle plugin, int numParams)
{
	...
	RemoveCredits(GetNativeCell(1), GetNativeCell(2), view_as<Store_RemoveCreditsCallback>(GetNativeFunction(3)), plugin, data, szLogReason);
}
...
public int Native_RemoveCreditsFromUsers(Handle plugin, int numParams)
{
	...
	RemoveCreditsFromUsers(accountIds, length, GetNativeCell(3), szLogReason);
}
...
public int Native_RemoveDifferentCreditsFromUsers(Handle plugin, int numParams)
{
	...
	RemoveDifferentCreditsFromUsers(accountIds, length, credits, szLogReason);
}
