...
public int Native_GetAccessFlag(Handle hPlugin, int argc)
{
	return g_iFlags;
}
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("tVip_IsVip", NativeIsVip);
	CreateNative("tVip_GetAccessFlag", Native_GetAccessFlag);
	
	RegPluginLibrary("tVip");
...
public void OnPluginStart() {
	...
	g_hFlag = AutoExecConfig_CreateConVar("tVip_flag", "t", "Flags to give VIP");
	...
	RegAdminCmd("sm_viplist", cmdListVips, ADMFLAG_ROOT, "Shows all VIPs");
	
	RegConsoleCmd("sm_myvip", openVipPanel);
	
	g_hForward_OnClientLoadedPost = CreateGlobalForward( "tVip_OnClientLoadedPost", ET_Event, Param_Cell, Param_Cell);
...
public void OnConfigsExecuted() {
	...
	g_iFlags = ReadFlagString(cFlags);
...
public Action openVipPanel(int client, int args)
{
	if (g_bIsVip[client])
	{
		...
		*/
		...
		if(StrContains(playerid, "STEAM_1") != -1)
		{
			//strcopy(szOtherPlayerId, sizeof szOtherPlayerId, playerid);
			ReplaceString(playerid, sizeof playerid, "STEAM_1", "STEAM_0");
		}
	...
	{
		CPrintToChat(client, "\x01You are not a \x05VIP\x01! You can become one by going to the following website: \x05%s", VIP_PURCHASE_WEBSITE);
	}
...
public Action cmdAddVip(int client, int args) {
	if (args < 3)
	{
		if (client != 0)
		{
			CPrintToChat(client, "{olive}[VIP] {lightred}Invalid Params Usage: sm_addvip \"<SteamID>\" <Duration in Month> \"<Name>\" [0=Month,1=Minutes]");
		}
		
		else
		{
			PrintToServer("[VIP] Invalid Params Usage: sm_addvip \"<SteamID>\" <Duration in Month> \"<Name>\"");
		}
		return Plugin_Handled;
	}
	
	char input[MAX_AUTHID_LENGTH + 4];
	...
	*/
	...
	if(StrContains(input, "STEAM_1", false) != -1)
	{
		ReplaceString(input, sizeof input, "STEAM_1", "STEAM_0");
	}
...
public Action cmdtVIP(int client, int args)
{
	...
	SetMenuTitle(mainChooser, "tVIP Control");
...
public void grantVip(int admin, int client, int duration, int reason) {
	...
	if(StrContains(admin_playerid, "STEAM_1") != -1)
	{
		ReplaceString(admin_playerid, sizeof admin_playerid, "STEAM_1", "STEAM_0");
	}
...
public void grantVipEx(int admin, char[] playerid, int duration, char[] pname, int timeFormat) {
	char admin_playerid[20];
	if (admin != 0) {
		GetClientAuthId(admin, AuthId_Steam2, admin_playerid, sizeof(admin_playerid));
...
void CallVIPLoadedForward(int client, bool bIsVIP)
{
	Call_StartForward(g_hForward_OnClientLoadedPost);
	Call_PushCell(client);
	Call_PushCell(bIsVIP);
	Call_Finish();
}
...
public void loadVip(int client)
{
	...
	if( GetUserFlagBits(client) & ADMFLAG_ROOT )
	{
		...
		CallVIPLoadedForward(client, true);
		
		return;
	}
	
	char playerid[MAX_AUTHID_LENGTH];
	GetClientAuthId(client, AuthId_Steam2, playerid, sizeof(playerid));
...
public void SQLCheckVIPQuery(Handle owner, Handle hndl, const char[] error, any data)
{
	...
	CallVIPLoadedForward(client, g_bIsVip[client]);
}

public void setFlags(int client)
{
	g_bIsVip[client] = true;
	SetUserFlagBits(client, GetUserFlagBits(client) | g_iFlags);
}
...
public void deleteVip(char[] playerid)
{
	...
	FormatEx(deleteVipQuery, sizeof(deleteVipQuery), "UPDATE tVip SET `endtime` = NOW() WHERE playerid = '%s';", playerid);
...
public void extendVip(int client, int userTarget, int duration)
{
	...
	char clean_playername[MAX_NAME_LENGTH + 8];
	SQL_EscapeString(g_DB, playername, clean_playername, sizeof(clean_playername));
...
//Natives
public int NativeIsVip(Handle hPlugin, int argc)
{
	return g_bIsVip[GetNativeCell(1)];
}
...
public int NativeDeleteVip(Handle myplugin, int argc)
{
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		...
		ReplaceString(szClientAuthId, sizeof szClientAuthId, "STEAM_1", "STEAM_0");
		
		if(StrEqual(szClientAuthId, playerid))
		{
			g_bIsVip[client] = false;
			CallVIPLoadedForward(client, false);
