...
public void OnAllPluginsLoaded()
{
	...
	VIPSys_Menu_AddItem("knivesmenu", "Select Knives Menu", MenuAction_Select, ITEMDRAW_DEFAULT, VIPMenuHandler_OnKnivesMenu, 5);
	VIPSys_Menu_AddItem("paintsmenu", "Weapon Skins Menu", MenuAction_Select, ITEMDRAW_DEFAULT, VIPMenuHandler_OnPaintsMenu, 7);
}
...
public void OnPluginEnd()
{
	...
	VIPSys_Menu_RemoveItem("knivesmenu");
	VIPSys_Menu_RemoveItem("paintsmenu");
}
...
public int VIPMenuHandler_OnKnivesMenu(Menu menu, char[] szInfo, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		if( !IsClientVIP(param1) )
		{
			CPrintToChat(param1, "\x04%s \x02You are not a \x05VIP\x02.", g_ChatPrefix);
			return;
		}
		
		CommandKnife(param1, 0);
	}
}

public int VIPMenuHandler_OnPaintsMenu(Menu menu, char[] szInfo, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		if( !IsClientVIP(param1) )
...
public Action CommandWeaponSkins(int client, int args)
{
	if ( IsValidClient(client) )
	{
		...
		{
			CPrintToChat(client, "\x04%s \x02You are not a \x05VIP\x02.", g_ChatPrefix);
