	...
	*/
	if (g_bTimeractivated[client])
	{
		...
		if(!StrEqual(g_szTimerTitle[client],""))
		{
			...
			SetHudTextParams(POS_X, POS_Y, HOLD_TIME, COLOR_R, COLOR_G, COLOR_B, COLOR_A);
			ShowSyncHudText(client, g_SyncHud, "%s\n\nSpeed: %.1f u/s\n%s", g_szPlayerPanelText[client], GetSpeed(client), g_szTimerTitle[client]);
			//ShowHudText(client, HUD_CHAN, "Speed: %.1f u/s\n", GetSpeed(client), g_szTimerTitle[client]);
			//PrintHintText(client, g_szTimerTitle[client]);
		}
	}
	else
	{
		...
		SetHudTextParams(POS_X, POS_Y, HOLD_TIME, COLOR_R, COLOR_G, COLOR_B, COLOR_A);
		ShowSyncHudText(client, g_SyncHud, szTmp);
		//ShowHudText(client, HUD_CHAN, szTmp);
		//PrintHintText(client, szTmp);
...
public SpecList(client)
{
	...
	*/
	if(!StrEqual(g_szPlayerPanelText[client],""))
	{
		...
		SetHudTextParams(POS_X, POS_Y, HOLD_TIME, COLOR_R, COLOR_G, COLOR_B, COLOR_A);
		ShowSyncHudText(client, g_SyncHud, g_szPlayerPanelText[client]);
		//ShowHudText(client, HUD_CHAN, g_szPlayerPanelText[client]);
		//PrintHintText(client, g_szPlayerPanelText[client]);
	}
}
...
public PerformBan(client, String:szbantype[16])
{
	if (IsValidClient(client))
	{
		...
		if (SOURCEBANS_AVAILABLE())
			SourceBans_BanPlayer(0, client, duration, szbantype);
		else
	...
	if (!g_bTimeractivated[client])
	{
		...
		else
		{
			...
			//else
				//PlayerPanel(client);
				// Changed it to down
				// Keep it activated all the time
		}
	}
...
public RegServerConVars()
{
	...
	g_hEnableGroupAdverts = CreateConVar("kz_steamgroup_advert", "0", "Enable or disable KZTimer's steamgroup adverts", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public SetupExceptions(bool:addcommands)
{
	...
	while(!IsEndOfFile(fileHandle)&&ReadFileLine(fileHandle,line,sizeof(line)))
	{
		...
		if ((StrContains(line,"//",true) == -1))
		{
			...
			if (!StrEqual(line,""))
			{
				if(StrContains(line, "say", false) != -1)
				{
					ReplaceString(line, sizeof line, "say", "", false);
					TrimString(line);
					SetTrieValue(g_Trie_Exceptions_ChatCommands, line, 1);
				}
				
				else if(StrContains(line, "say_team", false))
				{
					ReplaceString(line, sizeof line, "say_team", "", false);
					TrimString(line);
					SetTrieValue(g_Trie_Exceptions_ChatCommands, line, 1);
				}
				
				else if(StrContains(line, "sm_", false))
				{
					PrintToServer("szCmd: %s", line);
					if (addcommands)
					{
					...
					ReplaceString(line, sizeof line, "sm_", "", false);
					
					FormatEx(szCmd, sizeof szCmd, "!%s", line);
					PrintToServer("szCmd: %s", line);
					SetTrieValue(g_Trie_Exceptions_ChatCommands, szCmd, 1);
	...
	if(!addcommands)
	{
		ClearTrie(g_Trie_Exceptions_ChatCommands);
	}
