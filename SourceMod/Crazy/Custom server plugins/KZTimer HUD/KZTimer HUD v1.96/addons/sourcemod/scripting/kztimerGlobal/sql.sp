...
public db_DeleteCheater(String:steamid[32])
{
	...
	SQL_EscapeString(g_hDb, steamid, szsteamid, 32*2+1);      	
...
public db_InsertLatestRecords(String:szSteamID[32], String:szName[MAX_NAME_LENGTH], Float: FinalTime, Teleports)
{
	decl String:szQuery[512];       
		...
		if (StartCords[0] != -1.0 && StartCords[1] != -1.0 && StartCords[2] != -1.0)
		{
			...
			{  
				...
				for (int index = 0; index <= MaxClients; index++)
				{
					...
					g_bFirstStartButtonPush[index] = false;
				}
		...
		{		
			...
			{  
				...
				for (int index = 0; index <= MaxClients; index++)
				{
					...
					g_bFirstEndButtonPush[index] = false;
				}
