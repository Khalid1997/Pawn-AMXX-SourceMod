...
public KzAdminMenu(client)
{
	...
	if (g_iNoClipMode == NOCLIPMODE_DISABLED)
		Format(szTmp, sizeof(szTmp), "[21.] +noclip  -  Disabled");
	else if (g_iNoClipMode == NOCLIPMODE_NORMAL)
		Format(szTmp, sizeof(szTmp), "[21.] +noclip  -  Enabled for VIP/top rank/map finished");
	else if (g_iNoClipMode == NOCLIPMODE_PRIVILEGED)
		Format(szTmp, sizeof(szTmp), "[21.] +noclip  -  Enabled for only VIP/top rank");
	else
		Format(szTmp, sizeof(szTmp), "[21.] +noclip  -  Enabled for everyone");
	AddMenuItem(adminmenu, szTmp, szTmp);
	...
	if (g_iDoubleDuckCvar == 1)
		...
		Format(szTmp, sizeof(szTmp), "[34.] Double-Duck technique  -  Disabled in runs"); 			
	else
