...
{
	name = "SM Franug Auto Respawn_ Modified",
...
public Action Event_Playerdeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if(enable)
	{
		CreateTimer(g_respawn, Respawn, GetClientUserId(client));
	}
}
...
public Action Respawn(Handle timer, int userid)
{
	...
	if(IsClientInGame(client) && !IsPlayerAlive(client) && GetClientTeam(client) > 1 && enable)
	{
...
public void Event_RoundEnd(Event event, const char[] szName, bool bDontBroadcast)
{
	enable = false;
