...
new g_iBlocks[MAX_EDICT] =  { -1, ... };
new g_iTeleporters[MAX_EDICT] =  { -1, ... };
...
new Float:TrampolineForce[MAX_EDICT] = 0.0
new Float:SpeedBoostForce_1[MAX_EDICT] = 0.0
new Float:SpeedBoostForce_2[MAX_EDICT] = 0.0
//new Float:velocity_duck = 0.0
new Block_Transparency[MAX_EDICT] = 0
//new blocksize = 0
...
new bool:g_bTriggered[MAX_EDICT] =  { false, ... };
...
new bool:g_bGroups[MAXPLAYERS + 1][MAX_EDICT];
new bool:g_bRandomCantUse[MAXPLAYERS + 1]

Handle Block_Timers[MAXPLAYERS] = { null, ... };
...
public OnPluginStart()
{
	...
	Format(CHAT_TAG, sizeof(CHAT_TAG), "[BlockMaker]")
...
public Action:Command_GrabBlock(client, args)
{
	if (g_iDragEnt[client] == 0)
	{
		...
		float vEyePos[3], vAngles[3];
		GetClientEyeAngles(client, vAngles);
		GetClientEyePosition(client, vEyePos);
		
		tr = TR_TraceRayFilterEx(vEyePos, vAngles, MASK_PLAYERSOLID, RayType_Infinite, TraceFilter, client);
		
		if(!TR_DidHit(tr))
		{
			delete tr;
			PrintToChat(client, "did not hit anything");
		...
		if (IsValidBlock(ent))
		{
			...
			g_flOffset[client] = GetEntityDistance(client, ent);
			
			if(g_bGroups[client][g_iDragEnt[client]])
			{
				for(new i = MaxClients + 1; i < MAX_EDICT; i++)
				{
...
float GetEntityDistance(int client, int iEnt)
{
	float vOrigin1[3], vOrigin2[3];
	GetClientEyePosition(client, vOrigin1);
	GetEntPropVector(iEnt, Prop_Send, "m_vecOrigin", vOrigin2);
	
	//PrintToChatAll("%0.2f -- vOrigin: %0.2f %0.2f %0.2f", GetVectorDistance(vOrigin1, vOrigin2, false), vOrigin2[0], vOrigin2[1], vOrigin2[2])
	
	return GetVectorDistance(vOrigin1, vOrigin2, false);
}
...
public Action:Command_ReleaseBlock(client, args)
{
	if (g_iDragEnt[client] != 0)
	{
		...
		if(CheckBlockStuck(g_iDragEnt[client], client))
		{
			AcceptEntityInput(g_iDragEnt[client], "Kill");
			PrintToChat(client, "Removed block as it is stuck.");
		}
...
bool CheckBlockStuck(int ent, int client = 0)
{
	if(client)
	{
	...
	//float vCOrigin[3]; GetClientAbsOrigin(client, vCOrigin);
	float vOrigin[3];
	float vMin[3], vMax[3];
	float vFwd[3], vRight[3], vUp[3];
	float vPoint[3];
	GetEntPropVector(ent, Prop_Send, "m_vecMins", vMin);
	GetEntPropVector(ent, Prop_Send, "m_vecMaxs", vMax);
	
	GetEntPropVector(ent, Prop_Send, "m_angRotation", vOrigin);
	GetAngleVectors(vOrigin, vFwd, vRight, vUp);
	...
	int iTotalStuckPoints;
	// --------  Start
	ScaleVectorEx(vOrigin, 1.0, vPoint, false);
	ScaleVectorEx(vFwd, vMax[0], vPoint, true);
	ScaleVectorEx(vRight, vMax[1], vPoint, true);
	ScaleVectorEx(vUp, vMax[2], vPoint, true);
	
	//SendPointLaser(vPoint, vCOrigin);
	
	if(TR_GetPointContents(vPoint) != CONTENTS_EMPTY)
	{
		iTotalStuckPoints++;
	}
	...
	ScaleVectorEx(vFwd, vMax[0], vPoint, true);
	ScaleVectorEx(vRight, vMax[1], vPoint, true);
	ScaleVectorEx(vUp, vMin[2], vPoint, true);
	//SendPointLaser(vPoint, vCOrigin);
	if(TR_GetPointContents(vPoint) != CONTENTS_EMPTY)
	{
		iTotalStuckPoints++;
	...
	ScaleVectorEx(vOrigin, 1.0, vPoint, false);
	ScaleVectorEx(vFwd, vMax[0], vPoint, true);
	ScaleVectorEx(vRight, vMin[1], vPoint, true);
	ScaleVectorEx(vUp, vMax[2], vPoint, true);
	//SendPointLaser(vPoint, vCOrigin);
	if(TR_GetPointContents(vPoint) != CONTENTS_EMPTY)
	{
	...
	ScaleVectorEx(vOrigin, 1.0, vPoint, false);
	ScaleVectorEx(vFwd, vMin[0], vPoint, true);
	ScaleVectorEx(vRight, vMax[1], vPoint, true);
	ScaleVectorEx(vUp, vMax[2], vPoint, true);
	//SendPointLaser(vPoint, vCOrigin);
	if(TR_GetPointContents(vPoint) != CONTENTS_EMPTY)
	...
	PrintToChatAll("Stuck: %d", iTotalStuckPoints);
	if(iTotalStuckPoints >= 8)
	{
		return true;
	}
...
void ScaleVectorEx(float vec[3], float scale, float vecoutput[3], bool bAdd = true)
{
	if(bAdd)
	{
		vecoutput[0] += vec[0] * scale;
		vecoutput[1] += vec[1] * scale;
		vecoutput[2] += vec[2] * scale;
	}
...
stock void SendPointLaser(float vOrigin1[3], float vOrigin2[3])
{
	float life = 5.0,
	width = 1.0,
	end = 1.0,
	amp = 0.0;
	int length = 1;
	TE_SetupBeamPoints(vOrigin1, vOrigin2, g_Model, 0, 0, 0, life, width, end, length, amp, { 255, 255, 255, 255 }, 0);
	TE_SendToAll();
}
...
public OnMapStart()
{
	//g_Model = PrecacheModel("materials/sprites/laserbeam.vmt");
...
public Action:Command_SaveBlocks(client, args)
{
	...
	for (new i = MaxClients + 1 ; i < MAX_EDICT; i++)
	{
		...
		else
		{
			...
			if (g_iBlocks[i] == 5 || g_iBlocks[i] == 35 || g_iBlocks[i] == 64 || g_iBlocks[i] == 93)
			{
				KvSetFloat(g_hBlocksKV, "attrib1", TrampolineForce[i])
			}
			else if (g_iBlocks[i] == 6 || g_iBlocks[i] == 36 || g_iBlocks[i] == 65 || g_iBlocks[i] == 94)
			{
				KvSetFloat(g_hBlocksKV, "attrib1", SpeedBoostForce_1[i])
				KvSetFloat(g_hBlocksKV, "attrib2", SpeedBoostForce_2[i])
			}
			else if (g_iBlocks[i] == 28 || g_iBlocks[i] == 57 || g_iBlocks[i] == 86 || g_iBlocks[i] == 115 || g_iBlocks[i] == 18 || g_iBlocks[i] == 47 || g_iBlocks[i] == 76 || g_iBlocks[i] == 105 || g_iBlocks[i] == 1 || g_iBlocks[i] == 31 || g_iBlocks[i] == 60 || g_iBlocks[i] == 89)
			{
				KvSetFloat(g_hBlocksKV, "attrib1", SpeedBoostForce_1[i])
			}
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	for (new a = MaxClients + 1; a < MAX_EDICT; a++)
	{
		if (g_iBlocks[a] == 10)
		{
	...
	if (g_iDragEnt[client] != 0)
	{
		if (IsValidEdict(g_iDragEnt[client]))
		{
			...
			vecPos[0] += vecDir[0] * g_flOffset[client];
			vecPos[1] += vecDir[1] * g_flOffset[client];
			vecPos[2] += vecDir[2] * g_flOffset[client];
			...
			if (g_bSnapping[client] && (FloatAbs(g_fClientAngles[client][1]) - FloatAbs(angles[1])) < 2.0 && !bGroup)
			{
				for (int i = MaxClients + 1; i < MAX_EDICT; i++)
				{
					if (IsValidBlock(i) && i != g_iDragEnt[client])
					{
						...
						if (GetVectorDistance(/* vecDir */vecPos, fPos3) <= 60.0 + g_fSnappingGap[client])
						{
							bSnap = true;
			...
			if(!bSnap)
			{
				...
				ScaleVector(vecVel, BLOCK_MOVE_SPEED);
				TeleportEntity(g_iDragEnt[client], vecPos, g_fAngles[g_iDragEnt[client]], NULL_VECTOR);
				...
				if(bGroup)
				{
					...
					for(new i = MaxClients + 1; i < MAX_EDICT; i++)
					{
						if(IsValidBlock(i) && i != g_iDragEnt[client] && g_bGroups[client][i])
						{
							...
							TeleportEntity(i, vecPos, g_fAngles[i], NULL_VECTOR);
						}
					}
				}
			}
	...
	if (action == MenuAction_Select)
	{
		...
		} else if (param2 == 2)
		{
			...
			if (IsValidBlock(ent))
			{
				...
				if(CheckBlockStuck(ent, client))
				{
					AcceptEntityInput(ent, "Kill");
					PrintToChat(client, "Deleted block as it got stuck");
				}
			}
			else
			{
...
CreateBlock(client, blocktype = 0, Float:fPos[3] =  { 0.0, 0.0, 0.0 }, Float:fAng[3] =  { 0.0, 0.0, 0.0 }, Float:attrib1 = 0.0, Float:attrib2 = 0.0, transparency = 0)
{
	...
	{
		SetEntProp(block_entity, Prop_Data, "m_CollisionGroup", 4);
		//PrintToChatAll("Made Solid");
	}
...
public Handler_Options(Handle:menu, MenuAction:action, client, param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			PrintToChat(client, "OK, this is broken, and I dont want to fix it cause it will most likely lag the server if you have a big group, so no.");
