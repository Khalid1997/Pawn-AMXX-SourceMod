...
bool g_bScoutCanUse[MAXPLAYERS + 1];
bool g_bShotgunCanUse[MAXPLAYERS + 1];
bool g_bP250CanUse[MAXPLAYERS + 1];
...
float g_flDragDistance[MAXPLAYERS + 1];

// Simulate ICE Effect!!!
bool g_bOnIce[MAXPLAYERS + 1];

ConVar ConVar_Friction;
ConVar ConVar_Accelerate;

float g_flFriction;
float g_flAccelerate;

#define ON_ICE_FRICTION 0.15
#define ON_ICE_ACCELERATE 1.25

bool g_bIgnoreFriction = false;
bool g_bIgnoreAccelerate = false;
...
public void ConVarChange_Friction(ConVar convar, char[] szNewValue, char[] szOldValue)
{
	if(g_bIgnoreFriction)
	{
		return;
	}
	
	g_flFriction = ConVar_Friction.FloatValue;
	PrintToChatAll("Friction %0.2f", g_flFriction);
}

public void ConVarChange_Accelerate(ConVar convar, char[] szNewValue, char[] szOldValue)
{
	if(g_bIgnoreAccelerate)
	{
		return;
	}
	
	g_flAccelerate = ConVar_Accelerate.FloatValue;
	PrintToChatAll("Accelerate %0.2f", g_flAccelerate);
}

public void OnPluginPauseChange(bool bPause)
{
	switch (bPause)
	{
		case true:
		{
			ConVar_Friction.Flags = ConVar_Friction.Flags | (FCVAR_NOTIFY | FCVAR_REPLICATED);
			ConVar_Accelerate.Flags = ConVar_Friction.Flags | (FCVAR_NOTIFY | FCVAR_REPLICATED);
		}
...
{
	ConVar_Friction = FindConVar("sv_friction");
	ConVar_Accelerate = FindConVar("sv_accelerate");
	if (ConVar_Friction == null || ConVar_Accelerate == null)
	{
		SetFailState("Failed to find ConVar sv_friction or sv_accelerate!");
		return;
	}
	...
	ConVar_Friction.Flags = ConVar_Friction.Flags & ~(FCVAR_NOTIFY | FCVAR_REPLICATED);
	ConVar_Accelerate.Flags = ConVar_Accelerate.Flags & ~(FCVAR_NOTIFY | FCVAR_REPLICATED);
	
	HookConVarChange(ConVar_Friction, ConVarChange_Friction);
	HookConVarChange(ConVar_Accelerate, ConVarChange_Accelerate);
...
public Action Command_GrabBlock(int client, int args)
{
	if (g_iDragEnt[client] == 0)
	{
		...
		if (IsValidBlock(ent))
		{
			...
			float vPos[3], vBlockPos[3];
			GetClientEyePosition(client, vPos);
			GetEntPropVector(ent, Prop_Send, "m_vecOrigin", vBlockPos);
			
			g_flDragDistance[client] = GetVectorDistance(vBlockPos, vPos);
...
public Action RoundStart(Handle event, char[] name, bool dontBroadcast)
{
	...
	for (int i = 1; i <= MaxClients; ++i)
	{
		...
		g_bScoutCanUse[i] = true;
		g_bP250CanUse[i] = true;
		g_bShotgunCanUse[i] = true;
	}
...
public void Event_PreThinkPost_Client(int client)
{
	if (g_bOnIce[client])
	{
		g_bIgnoreFriction = true;
		g_bIgnoreAccelerate = true;
		
		SetConVarFloat(ConVar_Friction, ON_ICE_FRICTION);
		SetConVarFloat(ConVar_Accelerate, ON_ICE_ACCELERATE);
		
		g_bIgnoreFriction = false;
		g_bIgnoreAccelerate = false;
	}
	...
	else
	{
		...
		SetConVarFloat(ConVar_Friction, g_flFriction);
		SetConVarFloat(ConVar_Accelerate, g_flAccelerate);
		
		g_bIgnoreFriction = false;
		g_bIgnoreAccelerate = false;
	}
	
	/*if (g_bOnIce[client] && IsPlayerAlive(client))
	{
...
void SetOnIceStatus(int client, bool bEnabled)
{
	if (bEnabled)
	{
		...
		char szValue[15];
		FormatEx(szValue, sizeof(szValue), "%0.5f", ON_ICE_FRICTION);
		SendConVarValue(client, ConVar_Friction, szValue);
		
		FormatEx(szValue, sizeof(szValue), "%0.5f", ON_ICE_ACCELERATE);
		SendConVarValue(client, ConVar_Accelerate, szValue);
		
		//PrintToChatAll("Enabled On Ice");
	}
	...
	else
	{
		...
		char szValue[15];
		FormatEx(szValue, sizeof(szValue), "%0.5f", g_flFriction);
		SendConVarValue(client, ConVar_Friction, szValue);
		
		FormatEx(szValue, sizeof(szValue), "%0.5f", g_flAccelerate);
		SendConVarValue(client, ConVar_Accelerate, szValue);
		
		//PrintToChatAll("Disabled On Ice");
	}
}
...
void ReadBlocksConfigFile()
{
	...
	do
	{
		...
		if (g_eBlocks[i][ModelName][0])
		{
			...
			//PrintToServer("Block %s: Included size normal", g_eBlocks[i][BlockName]);
		}
		
		if (g_eBlocks[i][ModelNameLarge][0])
		{
			iAvailableSizes |= Size_Large;
			//PrintToServer("Block %s: Included size large", g_eBlocks[i][BlockName]);
		}
		
		if (g_eBlocks[i][ModelNameSmall][0])
		{
			iAvailableSizes |= Size_Small;
			//PrintToServer("Block %s: Included size small", g_eBlocks[i][BlockName]);
		}
		
		if (g_eBlocks[i][ModelNamePole][0])
		{
			iAvailableSizes |= Size_Pole;
			//PrintToServer("Block %s: Included size pole", g_eBlocks[i][BlockName]);
		}
...
void SaveBlocks(bool msg = false)
{
	...
	for (int i = MaxClients + 1; i <= MAX_EDICTS; ++i)
	{
		...
		else
		{
			...
			KvSetNum(g_hBlocksKV, "blocksize", blocksize);
...
void LoadBlocks(bool msg = false)
{
	...
	do
	{
		...
		else
		{
			...
			int blocksize = KvGetNum(g_hBlocksKV, "blocksize");
			
			switch (blocksize)
			{
				case 1:blocksize = Size_Normal;
				case 2:blocksize = Size_Large;
				case 3:blocksize = Size_Small;
				case 4:blocksize = Size_Pole;
			}
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	for (int a = MaxClients + 1; a < MAX_EDICTS; ++a)
	{
		...
		else if (g_iBlocks[a] == 19 || g_iBlocks[a] == 5 || g_iBlocks[a] == 6) // NOFALLDAMAGE for the NOFALLDMG Block TRAMPOLINE AND SPEEDBOOST
		{
			GetEntPropVector(a, Prop_Data, "m_vecOrigin", fPos2);
	...
	if (g_iDragEnt[client] != 0)
	{
		if (IsValidEdict(g_iDragEnt[client]))
		{
			...
			vecPos[0] += vecDir[0] * g_flDragDistance[client];
			vecPos[1] += vecDir[1] * g_flDragDistance[client];
			vecPos[2] += vecDir[2] * g_flDragDistance[client];
			...
			else
			{
				...
				TeleportEntity(g_iDragEnt[client], vecDir, NULL_VECTOR, NULL_VECTOR);
				g_iDragEnt[client] = 0
...
public int Handler_BlockBuilder(Handle menu, MenuAction action, int client, int param2Old)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		if (StrEqual(szInfo, "size"))
		{
			...
			do
			{
				g_iSelectedBlockSize[client] = g_iSelectedBlockSize[client] << 1;
				//PrintToChatAll("%d", 1<<4)
				if (g_iSelectedBlockSize[client] > Size_Pole)
				{
					
					//PrintToChatAll("Bigger than pole %d", g_iSelectedBlockSize[client]);
					g_iSelectedBlockSize[client] = Size_Normal;
				}
			...
			DisplayMenu(CreateMainMenu(client), client, MENU_TIME_FOREVER);
			return;
		}
...
public Handle CreateMainMenu(int client)
{
	...
	//PrintToChatAll("Checked? %d ## %d - %d - %d - %d", g_iSelectedBlockSize[client], Size_Normal, Size_Large, Size_Small, Size_Pole);
...
int CreateBlock(int client, int blocktype = 0, int blocksize = 0, float fPos[3] =  { 0.0, 0.0, 0.0 }, float fAng[3] =  { 0.0, 0.0, 0.0 }, float attrib1 = 0.0, float attrib2 = 0.0, int transparency = 0)
{
	...
	if (blocksize == Size_Normal)
	{
		...
		DispatchKeyValue(block_entity, "model", g_eBlocks[blocktype][ModelName]);
	}
	else if (blocksize == Size_Large)
	{
		//SetEntityModel(block_entity, g_eBlocks[blocktype][ModelNameLarge]);
		DispatchKeyValue(block_entity, "model", g_eBlocks[blocktype][ModelNameLarge]);
	}
	else if (blocksize == Size_Small)
	{
		//SetEntityModel(block_entity, g_eBlocks[blocktype][ModelNameSmall]);
		DispatchKeyValue(block_entity, "model", g_eBlocks[blocktype][ModelNameSmall]);
	}
	else if (blocksize == Size_Pole)
	{
		//SetEntityModel(block_entity, g_eBlocks[blocktype][ModelNamePole]);
		DispatchKeyValue(block_entity, "model", g_eBlocks[blocktype][ModelNamePole]);
	}
...
void CheckKillTimer(int client)
{
	...
	KillTimer(Block_Timers[client], false);
	Block_Timers[client] = null;
}

public Action OnStartTouch(int ent1, int ent2)
{
	int client = -1;
	int block = -1;
	...
	if (FL_ONGROUND && GetEntPropEnt(client, Prop_Send, "m_hGroundEntity") == block)
	{
		...
		if (g_iBlocks[block] == 24)
		{
			if (!g_bRandomCantUse[client])
			{
				...
				{
					GivePlayerCamo(client, block, true);
				}
				else if (random == 4)
				{
					Handle packet_f = CreateDataPack()
		...
		//		Block_Timers[client] = CreateTimer(g_eBlocks[3][EffectTime], HealPlayer, client);
		//	}
		else if (g_iBlocks[block] == 4) // ICE
		{
			//SetOnIceStatus(client, true);
		}
		...
		//	else if(g_iBlocks[block]==14 || g_iBlocks[block]O3 || g_iBlocks[block]==72 || g_iBlocks[block]==101)
		//	{
		//		if(GetClientTeam(client) == 2)
		//		{
		//			SetEntProp(block_entity, Prop_Data, "m_CollisionGroup", 2);
		...
		else if (g_iBlocks[block] == 29) // Scout
		{
			if (g_bScoutCanUse[client])
			{
				if (GetClientTeam(client) == 2)
				{
					int ent = -1;
					ent = Client_GiveWeaponAndAmmo(client, "weapon_ssg08", true, 0, 1, 1, 1);
					SetEntProp(ent, Prop_Data, "m_iClip1", 1);
					SetEntProp(ent, Prop_Data, "m_iClip2", 1);
					SetEntData(client, g_iAmmo + (GetEntData(ent, g_iPrimaryAmmoType) << 2), 0, 4, true);
					PrintToChatAll("\x03%s\x04 %N has got a SCOUT", CHAT_TAG, client);
					
					g_bScoutCanUse[client] = false;
				}
			}
		}
		
		else if (g_iBlocks[block] == 30) // p250
		{
			if (g_bP250CanUse[client])
			{
				if (GetClientTeam(client) == 2)
				{
					int ent = -1;
					ent = Client_GiveWeaponAndAmmo(client, "weapon_p250", true, 0, 1, 1, 1);
					SetEntProp(ent, Prop_Data, "m_iClip1", 1);
					SetEntProp(ent, Prop_Data, "m_iClip2", 1);
					SetEntData(client, g_iAmmo + (GetEntData(ent, g_iPrimaryAmmoType) << 2), 0, 4, true);
					PrintToChatAll("\x03%s\x04 %N has got a P250", CHAT_TAG, client);
					
					g_bP250CanUse[client] = false;
				}
			}
		}
		
		else if (g_iBlocks[block] == 31) // Shotgun
		{
			if (g_bShotgunCanUse[client])
			{
				if (GetClientTeam(client) == 2)
				{
					int ent = -1;
					ent = Client_GiveWeaponAndAmmo(client, "weapon_sawedoff", true, 0, 1, 1, 1);
					SetEntProp(ent, Prop_Data, "m_iClip1", 1);
					SetEntProp(ent, Prop_Data, "m_iClip2", 1);
					SetEntData(client, g_iAmmo + (GetEntData(ent, g_iPrimaryAmmoType) << 2), 0, 4, true);
					PrintToChatAll("\x03%s\x04 %N has got a SHOTGUN", CHAT_TAG, client);
					
					g_bShotgunCanUse[client] = false;
				}
			}
		}
	...
	if (g_iBlocks[block] == 1 && !g_bTriggered[block])
	{
		//PrintToChatAll("Start No Block Start Touch #1");
		g_bTriggered[block] = true;
		CreateTimer(g_eBlocks[1][EffectTime], StartNoBlock, block);
	}
	
	else if (g_iBlocks[block] == 18 && !g_bTriggered[block])
	{
		//PrintToChatAll("Start No Block Start Touch #2");
		g_bTriggered[block] = true;
		CreateTimer(g_eBlocks[18][EffectTime], StartNoBlock, block);
		SetEntPropFloat(client, Prop_Send, "m_flStamina", 0.0);
	}
	else if (g_iBlocks[block] == 28 && !g_bTriggered[block]) // Delayed
	{
		//PrintToChatAll("Start No Block Start Touch #3");
		g_bTriggered[block] = true;
		CreateTimer(SpeedBoostForce_1[block], StartNoBlock, block);
	}
}

void GivePlayerCamo(int client, int block, bool bRandom = false)
{
	int iPlayers[MAXPLAYERS], iCount;
	iCount = GetPlayers(iPlayers, GP_Flag_Alive, GetClientTeam(client) == 1 ? GP_Team_Second : GP_Team_First);
	
	if (!iCount)
	{
		PrintToChat(client, "\x03%s\x04* Camouflag only works if there is an ALIVE player in the other team! Aborting changing skins!", CHAT_TAG);
		return;
	}
	
	char szOldModel[PLATFORM_MAX_PATH], szNewModel[PLATFORM_MAX_PATH];
	int iSelectedPlayer;
	
	GetEntityModel(client, szOldModel, sizeof szOldModel);
	GetEntityModel((iSelectedPlayer = iPlayers[GetRandomInt(0, iCount - 1)]), szNewModel, sizeof szNewModel);
	
	SetEntityModel(client, szNewModel);
	...
	{
		PrintToChat(client, "\x03%s\x04 You are now camo! You have the same skin as \x04%N", CHAT_TAG, iSelectedPlayer);
	}
	...
	WritePackCell(packet_f, RoundIndex)
	WritePackCell(packet_f, client)
	WritePackString(packet_f, szOldModel);
	
	CreateTimer(g_eBlocks[21][EffectTime], ResetCamouflage, packet_f, TIMER_DATA_HNDL_CLOSE);
	CreateTimer(g_eBlocks[21][CooldownTime], ResetCamCanUse, packet_f, TIMER_DATA_HNDL_CLOSE);
...
public void OnTouch(int ent1, int ent2)
{
	...
	// Moved out of the player location if block to ensure that it starts again after 
	if (g_iBlocks[block] == 1 && !g_bTriggered[block])
	{
		//PrintToChatAll("OnTouch No Block Start Touch #1");
		g_bTriggered[block] = true;
		CreateTimer(g_eBlocks[1][EffectTime], StartNoBlock, block, TIMER_FLAG_NO_MAPCHANGE);
	}
	
	else if (g_iBlocks[block] == 18 && !g_bTriggered[block])
	{
		//PrintToChatAll("OnTouch No Block Start Touch #2");
		g_bTriggered[block] = true;
		CreateTimer(g_eBlocks[18][EffectTime], StartNoBlock, block, TIMER_FLAG_NO_MAPCHANGE);
		SetEntPropFloat(client, Prop_Send, "m_flStamina", 0.0);
	}
	else if (g_iBlocks[block] == 28 && !g_bTriggered[block]) // Delayed
	{
		//PrintToChatAll("OnTouch No Block Start Touch #3");
		g_bTriggered[block] = true;
		CreateTimer(SpeedBoostForce_1[block], StartNoBlock, block, TIMER_FLAG_NO_MAPCHANGE);
	}
	
	if (!(player_loc[2] <= block_loc[2]))
	{
		if (g_iBlocks[block] == 2)
		{
		} else if (g_iBlocks[block] == 31)
		{
...
public Action ResetCamouflage(Handle timer, any packet)
{
	...
	char szOldModel[PLATFORM_MAX_PATH];
	ReadPackString(packet, szOldModel, sizeof szOldModel);
	
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		SetEntityModel(client, szOldModel);
...
{
	//PrintToChatAll("Cancel no block");
