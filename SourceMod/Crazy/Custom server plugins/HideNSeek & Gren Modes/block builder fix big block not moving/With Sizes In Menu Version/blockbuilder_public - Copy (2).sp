...
public void ConVar_OnFrictionChanged(ConVar convar, char[] szNewValue, char[] szOldValue)
{
...
public void OnPluginPauseChange(bool bPause)
{
	switch (bPause)
	{
		case true:ConVar_Friction.Flags = ConVar_Friction.Flags | (FCVAR_NOTIFY | FCVAR_REPLICATED);
		case false:ConVar_Friction.Flags = ConVar_Friction.Flags & ~(FCVAR_NOTIFY | FCVAR_REPLICATED);
	}
}
...
public void OnPluginStart()
{
	...
	if (ConVar_Friction == null)
	{
		SetFailState("Failed to find ConVar sv_friction!");
		return;
	}
	
	ConVar_Friction.Flags = ConVar_Friction.Flags & ~(FCVAR_NOTIFY | FCVAR_REPLICATED);
	g_flFriction = ConVar_Friction.FloatValue;
	//HookConVarChange(ConVar_Friction, ConVar_OnFrictionChanged);
...
void SetOnIceStatus(int client, bool bEnabled)
{
	if (bEnabled)
	{
		...
		char szValue[15];
		FormatEx(szValue, sizeof(szValue), "%0.f", ON_ICE_FRICTION);
	...
	else
	{
		...
		char szValue[15];
		FormatEx(szValue, sizeof(szValue), "%0.f", g_flFriction);
		SendConVarValue(client, ConVar_Friction, szValue);
