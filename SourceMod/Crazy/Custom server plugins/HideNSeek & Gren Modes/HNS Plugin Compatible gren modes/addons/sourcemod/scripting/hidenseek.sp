...
public void OnCvarChange(ConVar hConVar, const char[] sOldValue, const char[] sNewValue)
{
	...
	else if (StrEqual("hns_countdown_time", sConVarName))
		g_fCountdownTime = StringToFloat(sNewValue);
	else if (StrEqual("hns_countdown_fade", sConVarName))
		g_bCountdownFade = hConVar.BoolValue;
	else if (StrEqual("hns_round_points", sConVarName))
		g_iRoundPoints = StringToInt(sNewValue);
	else if (StrEqual("hns_bonus_points_multiplier", sConVarName))
		g_iBonusPointsMultiplier = StringToInt(sNewValue);
	else if (StrEqual("hns_maximum_win_streak", sConVarName))
		g_iMaximumWinStreak = StringToInt(sNewValue);
	else if (StrEqual("hns_flashbang_chance", sConVarName))
		g_faGrenadeChance[NADE_FLASHBANG] = StringToFloat(sNewValue);
	else if (StrEqual("hns_molotov_chance", sConVarName))
		g_faGrenadeChance[NADE_MOLOTOV] = StringToFloat(sNewValue);
	else if(StrEqual("hns_smoke_grenade_chance", sConVarName))
		g_faGrenadeChance[NADE_SMOKE] = StringToFloat(sNewValue);
	else if (StrEqual("hns_decoy_chance", sConVarName))
		g_faGrenadeChance[NADE_DECOY] = StringToFloat(sNewValue);
	else if (StrEqual("hns_he_grenade_chance", sConVarName))
		g_faGrenadeChance[NADE_HE] = StringToFloat(sNewValue);
	else if (StrEqual("hns_flashbang_maximum_amount", sConVarName))
		g_iaGrenadeMaximumAmounts[NADE_FLASHBANG] = StringToInt(sNewValue);
	else if (StrEqual("hns_molotov_maximum_amount", sConVarName))
		g_iaGrenadeMaximumAmounts[NADE_MOLOTOV] = StringToInt(sNewValue);
	else if (StrEqual("hns_smoke_grenade_maximum_amount", sConVarName))
		g_iaGrenadeMaximumAmounts[NADE_SMOKE] = StringToInt(sNewValue);
	else if (StrEqual("hns_decoy_maximum_amount", sConVarName))
		g_iaGrenadeMaximumAmounts[NADE_DECOY] = StringToInt(sNewValue);
	else if (StrEqual("hns_he_grenade_maximum_amount", sConVarName))
		g_iaGrenadeMaximumAmounts[NADE_HE] = StringToInt(sNewValue);
	else if (StrEqual("hns_flash_blind_disable", sConVarName))
		g_iFlashBlindDisable = StringToInt(sNewValue);
	else if (StrEqual("hns_attack_while_frozen", sConVarName))
		g_bAttackWhileFrozen = hConVar.BoolValue;
	else if (StrEqual("hns_frostnades", sConVarName))
		g_bFrostNades = hConVar.BoolValue;
	else if (StrEqual("hns_self_freeze", sConVarName))
		g_bSelfFreeze = hConVar.BoolValue;
	else if (StrEqual("hns_freeze_glow", sConVarName))
		g_bFreezeGlow = hConVar.BoolValue;
	else if (StrEqual("hns_freeze_duration", sConVarName))
		g_fFreezeDuration = StringToFloat(sNewValue);
	else if (StrEqual("hns_freeze_fade", sConVarName))
		g_bFreezeFade = hConVar.BoolValue;
	else if (StrEqual("hns_frostnades_trail", sConVarName))
		g_bFrostNadesTrail = hConVar.BoolValue;
	else if (StrEqual("hns_freeze_radius", sConVarName))
		g_fFreezeRadius = StringToFloat(sNewValue);
	else if (StrEqual("hns_frostnades_detonation_ring", sConVarName))
		g_bFrostNadesDetonationRing = hConVar.BoolValue;
	else if (StrEqual("hns_block_console_kill", sConVarName))
		g_bBlockConsoleKill = hConVar.BoolValue;
	else if (StrEqual("hns_suicide_points_penalty", sConVarName))
		g_iSuicidePointsPenalty = StringToInt(sNewValue);
	else if (StrEqual("hns_molotov_friendly_fire", sConVarName))
		g_bMolotovFriendlyFire = hConVar.BoolValue;
	else if (StrEqual("hns_respawn_mode", sConVarName))
	{
	...
	else if (StrEqual("hns_base_respawn_time", sConVarName))
		g_fBaseRespawnTime = hConVar.FloatValue;
	else if (StrEqual("hns_respawn_invisibility_duration", sConVarName))
		g_fInvisibilityDuration = hConVar.FloatValue;
	else if (StrEqual("hns_invisibility_break_distance", sConVarName))
		g_fInvisibilityBreakDistance = hConVar.FloatValue + 64.0;
	else if (StrEqual("hns_ct_respawn_sleep_duration", sConVarName))
		g_fCTRespawnSleepDuration = hConVar.FloatValue;
	else if (StrEqual("hns_hide_radar", sConVarName))
		g_bHideRadar = hConVar.BoolValue;
	else if (StrEqual("hns_respawn_mode_roundtime", sConVarName))
		g_iRespawnRoundDuration = hConVar.IntValue;
	else if (StrEqual("hns_welcome_message", sConVarName))
		g_bWelcomeMessage = hConVar.BoolValue;
	else if (StrEqual("hns_damage_slowdown", sConVarName))
		g_bDamageSlowdown = hConVar.BoolValue;
	else if (StrEqual("hns_rm_save_respawn_points_to_file", sConVarName))
		g_iSaveSpawnPointsToFile = hConVar.IntValue;
	else if (StrEqual("hns_rm_load_respawn_points_from_file", sConVarName))
		g_bLoadSpawnPointsFromFile = hConVar.BoolValue;
	else if (StrEqual("hns_fast_knife", sConVarName))
		g_iFastKnife = hConVar.IntValue;
	else if (StrEqual("hns_wins_for_fast_knife", sConVarName))
		g_iWinsForFastKnife = hConVar.IntValue;
	else if (StrEqual("hns_grenade_no_block", sConVarName))
		g_bGrenadeNoBlock = hConVar.BoolValue;
	else if (StrEqual("hns_allow_weapons", sConVarName))
		g_bAllowWeapons = hConVar.BoolValue;
	else if (StrEqual("hns_adminmenu", sConVarName))
	{
		...
		if (topmenu != null)
		{
			...
			else topmenu.Remove(topmenu.FindCategory("hidenseek"));
		}	
	}
	else if (StrEqual("hns_score_crash_fix", sConVarName))
...
public Action DecoyDetonate(Handle hTimer, any iRef)
{
	...
	if (iEntity != INVALID_ENT_REFERENCE)
	{
		...
		int userid = GetClientUserId(GetEntPropEnt(iEntity, Prop_Send, "m_hThrower"));
		
		Handle event = CreateEvent("decoy_started");
		
		SetEventInt(event, "userid", userid);
		SetEventInt(event, "entityid", iEntity);
		SetEventFloat(event, "x", faDecoyCoord[0]);
		SetEventFloat(event, "y", faDecoyCoord[1]);
		SetEventFloat(event, "z", faDecoyCoord[2]);
		FireEvent(event);
	}
}

void DoFrostGrenadeStuff(int iThrower, float faDecoyCoord[3])
{
	EmitAmbientSound(SOUND_FROSTNADE_EXPLODE, faDecoyCoord, SNDLEVEL_NORMAL);
	//faDecoyCoord[2] += 32.0;
...
public Action Event_DecoyDetonate(Event event, const char[] name, bool dontBroadcast)
{
	...
	while ((index = FindEntityByClassname(index, "decoy_projectile")) != -1)
	{
		...
		if (xyz[0] == origin[0] && xyz[1] == origin[1] && xyz[2] == origin[2])
		{
			...
			iThrower = GetEntPropEnt(index, Prop_Send, "m_hThrower");
		}
	}
	
	DoFrostGrenadeStuff(iThrower, origin);
}
