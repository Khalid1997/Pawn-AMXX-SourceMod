...
char SIMON_NAME_C[MAX_SIMON_NAME_LENGTH] = "\x05Warden";
...
public APLRes AskPluginLoad2(Handle plugin, bool bLate, char[] error, int err_max)
{
	...
	CreateNative("SimonAPI_GetSimonName", Native_GetSimonName);
	CreateNative("SimonAPI_SetSimonName", Native_GetSimonName);
...
public int Native_GetSimonName(Handle hPlugin, int argc)
{
	SetNativeString(1, SIMON_NAME_C, GetNativeCell(1));
}

public int Native_SetSimonName(Handle hPlugin, int argc)
{
	char szName[35];
	GetNativeString(1, szName, sizeof szName);
	
	strcopy(SIMON_NAME_C, sizeof SIMON_NAME_C, szName);
}
