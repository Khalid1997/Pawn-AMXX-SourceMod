...
void GuideLaserToClient(int iEnt, int client)
{
	...
	TeleportEntity(EntRefToEntIndex(GetEntPropEnt(iEnt, Prop_Send, "m_hAttachEntity", 0)), vPosRight, 	NULL_VECTOR, vVelocity);
...
void CreateLasers(int iCount)
{
	...
	for (int i; i < iCount; i++)
	{
		...
		/*DispatchKeyValue(iEnt, "targetname", g_szBeamTargetName);
		DispatchKeyValue(iEnt, "damage", "100");
		DispatchKeyValue(iEnt, "framestart", "0");
		DispatchKeyValue(iEnt, "BoltWidth", "4.0");
		DispatchKeyValue(iEnt, "renderfx", "0");
		DispatchKeyValue(iEnt, "TouchType", "3"); // 0 = none, 1 = player only, 2 = NPC only, 3 = player or NPC, 4 = player, NPC or physprop
		DispatchKeyValue(iEnt, "framerate", "0");
		DispatchKeyValue(iEnt, "decalname", "Bigshot");
		DispatchKeyValue(iEnt, "TextureScroll", "35");
		DispatchKeyValue(iEnt, "HDRColorScale", "1.0");
		DispatchKeyValue(iEnt, "life", "0"); // 0 = infinite, beam life time in seconds
		DispatchKeyValue(iEnt, "StrikeTime", "1"); // If beam life time not infinite, this repeat it back
		DispatchKeyValue(iEnt, "spawnflags", "0"); // 0 disable, 1 = start on, etc etc. look from hammer editor
		DispatchKeyValue(iEnt, "NoiseAmplitude", "0"); // straight beam = 0, other make noise beam
		DispatchKeyValue(iEnt, "Radius", "256");
		...
		SetEntProp(iEndEnt, Prop_Data, "m_usSolidFlags", 0, 2);
		...
		//DispatchKeyValue(iEnt, "LightingStart", 
		...
		DispatchKeyValue(iEnt, "ScrollSpeed", "1");
		DispatchKeyValueFloat(iEnt, "NoiseAmplitude", 64.0);
		DispatchKeyValue(iEnt, "framestart", "1");
		//DispatchKeyValue(iEnt, "TouchType", "0");	// None
		
		char dum[10];
		FormatEx(dum, sizeof dum, "%d", BEAM_DAMAGE);
		DispatchKeyValue(iEnt, "damage", dum);
		...
		//FormatEx(szTargetName, sizeof szTargetName, g_szBeamTargetName);
		SetEntPropString(iEnt, Prop_Data, "m_iName", g_szBeamTargetName);
		...
		SetEntPropEnt(iEnt, Prop_Send, "m_hAttachEntity", EntIndexToEntRef(iStartEnt), 0);
		SetEntPropEnt(iEnt, Prop_Send, "m_hAttachEntity", EntIndexToEntRef(iEndEnt), 1);
