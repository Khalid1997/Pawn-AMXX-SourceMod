...
Handle g_hForward_Toggle, g_hChangeGlobalLock;

public int Native_SetGlobalLockMode(Handle hPlugin, int argc)
{
	ThirdPersonType iOld = g_iGlobalLockMode;
	...
	Call_StartForward(g_hChangeGlobalLock);
	{
		Call_PushCell(g_iGlobalLockMode);
		Call_PushCell(iOld);
		Call_Finish();
	}
}
...
public APLRes AskPluginLoad2(Handle hPlugin, bool bLate, char[] szError, int ErrMax)
{
	...
	g_hChangeGlobalLock = CreateGlobalForward("ThirdPerson_OnGlobalLockChanged", ET_Ignore, Param_Cell, Param_Cell);
