...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_advertisements_reload", Command_ReloadAds, ADMFLAG_ROOT, "Reload the advertisements");
}

void CreateAdvertisementsFile()
{
	char szPath[256];
	MultiMod_BuildPath(MultiModPath_Base, ModIndex_Null, szPath, sizeof szPath, "/advertisements.txt");
	
	if (!FileExists(szPath))
	{
		File f = OpenFile(szPath, "w");
		f.WriteLine("// Advertisements 2.0\n\
		// by Tsunami\n\
		//\n\
		// Types\n\
		// -----\n\
...
{
	ParseAds();
	ReplyToCommand(client, "* Advertisements Reloaded.");
...
void ParseAds()
{
	...
	MultiMod_BuildPath(MultiModPath_Base, ModIndex_Null, sPath, sizeof sPath, "/advertisements.txt");
