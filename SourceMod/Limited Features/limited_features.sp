...
public void Event_PlayerDisconnect(Event event, char[] szName, bool bDontBroadcast)
{
	...
	FormatQuery(szQuery, sizeof szQuery, "Update client time 1", "UPDATE {table_clients} SET {required_playtime_left} = \
		CASE \
		WHEN ( {required_playtime_left} - (%d - {playtime_offset}) ) < 0 THEN 0 \
		ELSE ( {required_playtime_left} - (%d - {playtime_offset}) ) END \
		WHERE {auth} = %d", 
...
void ConnectToSQLDatabase()
{
	...
	DBDriver driver = g_hSql.Driver;
	driver.GetIdentifier(szError, sizeof szError);
	
	#define MySQL 0
	#define SQLite 1
	int iDriver;
	
	if(StrEqual(szError, "sqlite"))
	{
		iDriver = SQLite;
	}
	...
	if(iDriver == MySQL)
	{
		...
		( {id} INTEGER PRIMARY KEY AUTO_INCREMENT, {feature_name} VARCHAR(32) NOT NULL UNIQUE, {feature_type} INTEGER NOT NULL, \
	...
	else
	{
		FormatQuery(szQuery, sizeof szQuery, "Make Feature Table", 
		"CREATE TABLE IF NOT EXISTS {table_features} \
		( {id} INTEGER PRIMARY KEY, {feature_name} VARCHAR(32) NOT NULL UNIQUE, {feature_type} INTEGER NOT NULL, \
			{reset_time_last} INTEGER NOT NULL, {reset_time} INTEGER NOT NULL, {uses_max} INTEGER NOT NULL, {required_playtime} INTEGER );");
	}
	...
	{
		FormatQuery(szQuery, sizeof szQuery, "Make Client Table", 
		"CREATE TABLE IF NOT EXISTS {table_clients} ( {id} INTEGER PRIMARY KEY, {auth} INTEGER NOT NULL, {feature_id} INTEGER NOT NULL, \
			{uses} INTEGER DEFAULT 0, {required_playtime_left} INTEGER, {playtime_offset} INTEGER DEFAULT 0 );");	
	}
...
void Fetch_RetrieveFeaturesResult(DBResultSet result)
{
	...
	{
		Func_CheckIfFeatureExpired(g_iFeatures[i][Feature_Id], true, INVALID_HANDLE, INVALID_FUNCTION, true, 0);
	}
}
...
void UpdateClientFeatures(int iFeatureId, int iDifferenceInPlayTime)
{
	...
	FormatQuery(szQuery, sizeof szQuery, "Update Client Features", 
		"UPDATE {table_clients} SET {required_playtime_left} = CASE \
		WHEN ({required_playtime_left} + %d) < 0 THEN 0 \
		ELSE ({required_playtime_left} + %d) END WHERE {feature_id} = %d", 
