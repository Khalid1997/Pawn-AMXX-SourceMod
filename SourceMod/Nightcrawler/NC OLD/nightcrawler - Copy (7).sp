...
char PLUGIN_CHAT_PREFIX[] = "\x04[NightCrawlers-Beta]";
...
float g_flNCStartingHealth = 160.0;
...
{
	PrintToChat_Custom(client, PLUGIN_CHAT_PREFIX, "You have activated the bomb! You will explode in \x04%0.1f \x01second!", g_flSuicideBombExplodeTime);
...
stock void MakeExplosion(int client, float vOrigin[3] =  { 0.0, 0.0, 0.0 } )
{
	...
	*/
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if( (flDistance = GetDistance(i, client) - 32.0 ) > flRadius)
		{
			//PrintToServer("1 %0.2f %0.2f", flDistance, flRadius);
			continue;
		}
		
		flClientDamage = ( 1.0 - ( flDistance  / flRadius ) ) * flDamage;
		//PrintToServer("Pass");
		//PrintToChatAll("Gave %N %0.2f damage", i, flClientDamage);
...
public Action Command_Join(int client, const char[] command, int argc)
{
	...
	{
		PrintToChat_Custom(client, PLUGIN_CHAT_PREFIX, "You cannot change teams.");
...
public void Frame_DoSpawnStuff(int iUserId)
{
	...
	{
		int iIndex = GetRandomInt(0, sizeof(g_szSurvivorPlayerModel) - 1);
		if (g_szSurvivorPlayerModel[iIndex][0])
		{
...
void PrintItemGiveChatMessages(int client, int iItem, bool bSuccess, int bFree, int iAdminId)
{
	switch (iItem)
	{
		...
		case ITEM_SUICIDEBOMB:
		{
			...
			PrintToChat_Custom(client, PLUGIN_CHAT_PREFIX, "You have the suicide bomb! Press +use key to activate it! (default: E)");
		}
...
public int MenuHandler_MainMenu(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		switch (StringToInt(szInfo))
		{
			...
			case 3:
			{
				if(GetClientTeam(param1) == CS_TEAM_NC)
				{
					bDisplayAgain = true;
					PrintToChat_Custom(param1, PLUGIN_CHAT_PREFIX, "You cannot join the queue while playing as a %s!", g_szNightcrawlerName);
				}
...
{
	#define PrintMethod PrintToChat
	PrintMethod(client, "-------------------------------");
	...
	PrintMethod(client, "You are a %s. You have to survive until the end of the round or kill all the %s.", g_szSurvivorName, g_szNightcrawlerTeamName);
	PrintMethod(client, "You can choose your guns, your (assisting) items, or buy upgrades from the shop.", g_szSurvivorName, g_szNightcrawlerTeamName);
	PrintMethod(client, "The %s are invisible. They only turn visible when you hurt them.", g_szNightcrawlerTeamName);
	PrintMethod(client, "-- As a %s:", g_szNightcrawlerName);
...
void ChooseNCPlayers()
{
	...
	PrintToChatAll("Ratio %0.2f - Calculated Count %d", g_flNCRatio, iNCCount);
...
void GiveLaser(int client, bool bStatus)
{
	switch (bStatus)
	{
		...
		case false:
		{
			...
			PrintToServer("Laser Entity: %d", g_iLaserEnt[client]);
			if (g_iLaserEnt[client] > 0 && IsValidEntity(g_iLaserEnt[client]))
			{
				int iEnt = g_iLaserEnt[client];
				//RemoveEdict(g_iLaserEnt[client]);
				//AcceptEntityInput(g_iLaserEnt[client], "KillHierarchy");
				
				//SetEntProp(iEnt, Prop_Send, "m_nNumBeamEnts", 2);
				//SetEntPropEnt(iEnt, Prop_Send, "m_hAttachEntity", EntIndexToEntRef(client), 0);
				//SetEntPropEnt(iEnt, Prop_Send, "m_hAttachEntity", 0, 1);
				//SetEntProp(iEnt, Prop_Send, "m_nBeamType", 0);
				
				//AcceptEntityInput(g_iLaserEnt[client], "TurnOff");
				AcceptEntityInput(iEnt, "Kill");
			}
...
void LoadKeyValues(Handle hKv, bool bInsertIntoHandle = false)
{
	enum
	{
	...
	for (int i; i < Key_Total; i++)
	{
		...
		{
			LogMessage("Key '%s' was not found in the config file.", Keys[i]);
		}
		else KvGoBack(hKv);
	}
}
