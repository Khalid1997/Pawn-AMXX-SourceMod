...
bool SetupMine(float position[3], float vnormal[3])
{
	...
	if (ent > -1)
	{
		...
		// create beam
		//TE_SetupBeamPoints(position, beamend, g_iBeamModelIndex, 0, 0, 0, 5.0, 5.0, 0.0, 1, 0.0, {255, 255, 255, 255}, 0);
		//TE_SendToAll();
...
public void Frame_DoSpawnStuff(int iUserId)
{
	...
	if (GetClientTeam(client) == CS_TEAM_NC)
	{
		...
		SetEntityRenderColor(client, 0, 255, 0, 64);
...
public int MenuHandler_ItemMenu(Menu menu, MenuAction action, int param1, int param2)
{
	...
	switch (action)
	{
		...
		{
			if(GetClientTeam(param2) != CS_TEAM_SURVIVOR)
			{
				return 0;
			}
...
public Action Timer_Detector(Handle hTimer, int client)
{
	...
	{
		//PrintToChatAll("In first loop %0.3f %0.3f", flDistance, g_flDetector_Radius);
		...
		//PrintToChatAll("Detector Text %s", szDetectorText);
	...
	//PrintToChatAll("%s", szDetectorText);
...
stock int GetNearestPlayer(int client, float &flDistance)
{
	...
	//PrintToChatAll("%N is nearest", iNearestPlayer);
...
void GiveGrenades(int client, bool bSet, int iCount)
{
...
void GiveLasers(int iHumans[MAXPLAYERS + 1], int iCount)
{
	DeleteLaserEntities();
