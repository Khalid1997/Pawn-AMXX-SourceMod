...
int g_iDefault_LaserMode = 							LMODE_RANDOM;
...
stock void MakeExplosion(int client, float vOrigin[3] =  { 0.0, 0.0, 0.0 }, bool bApplyForce = false, float flForce = 1000.0 )
{
	...
	*/
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		GetClientEyePosition(client, vOtherOrigin);
		...
		if(bApplyForce)
		{
			...
			if (vOtherOrigin[2] < flForce * flRatio)
			{
				vOtherOrigin[2] = flForce * flRatio;
			}
	...
	switch (action)
	{
		case MenuAction_DrawItem:
		{
			...
			if (iItemIndex == ITEM_LASER)
			{
				...
				{
					if(g_iLaserCarriersCount < g_iMaxLaserCount)
					{
...
void GiveLasersOnRoundStart()
{
	...
	g_iMaxLaserCount = RoundFloat(float(iCount) / g_flLaserRatio);
...
void GiveLaserToHighestKillsPlayers()
{
	...
	for(g_iLaserCarriersCount = 0; g_iLaserCarriersCount < g_iMaxLaserCount; g_iLaserCarriersCount++)
	{
		...
		PrintToServer("Given %N laser", iArray[0]);
	}
...
void GiveLaserToRandomPlayers()
{
	...
	if (!g_iMaxLaserCount && iCount)
	{
		g_iMaxLaserCount = 1;
	}
	
	int client;
	while (g_iLaserCarriersCount < g_iMaxLaserCount)
	{
	...
	PluginLog("iLaserCount: %d - iChosenCount: %d", g_iMaxLaserCount, g_iLaserCarriersCount);
}
