...
public Action Timer_Detector(Handle hTimer, int client)
{
	...
	if (iOtherClient && flDistance <= g_flDetector_Radius)
	{
		...
		//if (!(iHalfDashsNumber % 2))
		{
			...
			Format(szDetectorText, sizeof szDetectorText, "%s%s<font color=\"%s\">%s</font>%s%c", g_szDetectorDefaultMessage, szDashText, g_szDetectorCloseColor, szDetectorText, szDashText, (iHalfDashsNumber % 2) ? CHAR_DASH : 0);
		}
