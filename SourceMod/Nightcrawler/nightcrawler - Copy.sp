...
float g_flDefault_RoundWinSoundsVolume = 0.4;
float g_flDefault_AmbientRoundSoundsVolume = 1.0;
...
float g_flAmbientRoundSoundsVolume = 1.0;
...
void ActivateAdrenaline(int client)
{
	...
	EmitSoundToClientAny(client, g_szAdrenalineInjectionSound, SOUND_FROM_PLAYER, SOUNDCHANNEL_SURITEMS);
...
bool SetupMine(float position[3], float vnormal[3])
{
	...
	if (ent > -1)
	{
		...
		EmitSoundToAllAny(g_szMinePlacementSound, ent, SNDCHANNEL_MINE_SETUP);
		EmitSoundToAllAny(g_szMineArmingSound, ent, SNDCHANNEL_MINE_SETUP2);
...
{
	char szSound[PLATFORM_MAX_PATH];
	g_Array_AmbientRoundSounds.GetString(GetRandomInt(0, g_iAmbientRoundSoundsCount - 1), szSound, sizeof szSound);
	...
	for(int i; i < iCount; i++)
	{
		PrintToServer("Play Sound %N", iPlayers[i]);
		EmitSoundToClientAny(iPlayers[i], szSound, SOUND_FROM_PLAYER, SNDCHAN_AMBIENT_SOUNDS, _, _, g_flAmbientRoundSoundsVolume);
		//EmitAmbientSoundAny();
	}
}
...
public Action SDKHookCallback_OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	{
		EmitSoundToAllAny(g_szNightcrawlerKnifeHitSound[GetRandomInt(0, sizeof(g_szNightcrawlerKnifeHitSound) - 1)], attacker, SNDCHAN_WEAPON);
...
bool TeleportClient(int client)
{
	...
	EmitSoundToAllAny(g_szTeleportSound, client, SNDCHAN_STATIC);
...
void LoadKeyValues(KeyValues hKv, bool bInsertIntoHandle = false)
{
	...
	if(bInsertIntoHandle)
	{
		...
		KvSetFloat(hKv, Keys[Key_AmbientRoundSoundsVolume], g_flDefault_AmbientRoundSoundsVolume);
	...
	g_flAmbientRoundSoundsVolume = KvGetFloat(hKv, Keys[Key_AmbientRoundSoundsVolume], g_flDefault_AmbientRoundSoundsVolume);
...
void LoadDefaultValues()
{
	...
	g_flAmbientRoundSoundsVolume = g_flDefault_AmbientRoundSoundsVolume;
}
