...
int g_iClientAdminIndex[MAXPLAYERS] = No_Admin;
bool g_bLate = false;
...
{
	if(g_iClientAdminIndex[client] == -1)
	{
		if(g_iVIPIndex != -1)
		{
			if(bIsVIP)
			{
				g_iClientAdminIndex[client] = g_iVIPIndex;
			}
		}
	}
}
...
void ReloadConfig()
{
	...
	do
	{
		...
		for (int i; i < iCount; i++)
		{
			...
			#if defined INCLUDE_VIP
			if(StrContains(szStrings[i], "vip", false) != -1)
			{
				g_iVIPIndex = iEntry;
			}
			#endif
