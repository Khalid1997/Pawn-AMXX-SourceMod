...
stock hnsxp_set_user_xp(id, xp) { }
stock hnsxp_get_user_xp(id) { }
...
IsUnderAttack(id, iEnt)
{
	...
	static Float:vOrigin[Coord][3], Float:vEnd[3];
	new Float:vFakeEntOrigin[3], Float:vEntOrigin[Coord][3];
	
	pev(id, pev_origin, vOrigin[C_ORIGIN]);
	
	vEnd[2] = vOrigin[2][C_ORIGIN] + ( -1337 );
	
	pev(iEnt, pev_origin, vEntOrigin[C_ORIGIN]);
	
	vFakeEntOrigin[2] = vEnd[2] + 1337;
	
	vEnd[1] = ( vFakeEntOrigin[1] = vEntOrigin[C_ORIGIN][1] );
	vEnd[0] = ( vFakeEntOrigin[0] = vEntOrigin[C_ORIGIN][0] );
	
	static iHitEnt;
	engfunc(EngFunc_TraceLine, vFakeEntOrigin, vEnd, DONT_IGNORE_MONSTERS, id, iTr);
	...
	//static Float:vPlayerMins[3], Float:vPlayerMaxs[3];
	static Float:vBlockMins[3], Float:vBlockMaxs[3];
	static Float:vBlockOrigin[3]
	
	pev(id, pev_mins, vOriginC_MINS]);
	pev(id, pev_maxs, vOrigin[C_MAXS]);
	
	pev(iEnt, pev_mins, vEntOrigin[C_MINS]);
	pev(iEnt, pev_maxs, vEntOrigin[C_MAXS]);
	
	xs_vec_add(vEntOrigin[C_MAXS], vEntOrigin[C_ORIGIN], vEntOrigin[C_MAXS]);
	xs_vec_add(vEntOrigin[C_MINS], vEntOrigin[C_ORIGIN], vEntOrigin[C_MINS]);
	
	xs_vec_add(vOrigin[C_MINS], vOrigin[C_ORIGIN], vOrigin[C_MINS]);
	xs_vec_add(vOrigin[C_MAX], vOrigin[C_ORIGIN], vOrigin[C_MAXS]);
	//          
	// ---------- 
	// | // Maxs |
	// |---------|   // Block
	...
	// Player is stuck checks
	
	server_print("********************");
	server_print("vBlockMaxs = %.2f", vEntOrigin[C_MAXS][2]);
	server_print("vPlayerMaxs = %.2f", vOrigin[C_MAXS][2]);
	server_print("vBlockMins = %.2f", vEntOrigin[C_MINS][2]);
	server_print("********************");
	
	/*static i, j
	for( i = 0; i < sizeof vOrigin; i++)
	{
		for(j = 0; j < 2; j++)
		{
			if( vEntOrigin[C_MAXS][j] > vOrigin[i][j] > vEntOrigin[C_MINS][j] )
			{
	...
	// Player origin is under the block
	if(vOrigin[2] < vBlockMins[2])
	{
		// Check if his head is in the block or up of it
		if(vBlockMaxs[2] > vPlayerMaxs[2] > vBlockMins[2] || vPlayerMaxs[2] > vBlockMaxs[2])
		{
			engfunc(EngFunc_TraceLine, vOrigin[C_MINS], vEnd, DONT_IGNORE_MONSTERS, id, iTr);
			
			if(get_tr2(iTr, TR_pHit) != iEnt)
			{
				return 
			server_print("Here");
			return 0;
	...
	ColorChat(id, TEAM_COLOR, "^4%s ^3%s Tried to underblock", gszPrefix, szName);
...
actionXP(id, OverrideTimer)
{
	...
	if (fTime >= gfXPNextUse[id] || OverrideTimer)
	{
		if ( get_user_team ( id ) == 1 )
		{
			...
			ColorChat(id, GREEN, "^x03 You have been given^x04 100 XP^x03!");
		}
...
actionShotgun(id)
{
	if (is_user_alive(id) && !ShotgunUsed[id] && get_user_team(id) == 1)
		...
		set_hudmessage(0, 100, 255, -1.0, -1.0, 0, 6.0, 4.0);
		show_hudmessage(0, "Be careful CT! %s is running around with a Shotgun!", shotgunname);
	}
}
...
toggleGodmode(id)
{
    ...
    if (get_user_flags(id) & BM_ADMIN_LEVEL)
    {
        ...
        else
        {
            ...
            ColorChat(0, GREEN, "^x03[staNioN-MM]^x04 %s^x01 Has^x04 Enabled^x01 godmode!", szPlayerName);
        }
    }
}
