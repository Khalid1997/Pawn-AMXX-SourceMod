...
public eNewRound()
{
	...
	for(new i; i < iNum; i++)
	{
		...
		for(new a = TASK_HUD; a < TASKS; a += 124)
		{
			iTaskId = a + iPlayer
...
stock ChooseNCPlayers(iPlayers[32], iNum, Slay = 0)
{
	...
	for(new i; i < iNum; i++)
	{
		...
		#if defined CHANGE_NC_EACH_ROUND
		if(cs_get_user_team(iPlayer) == NC_TEAM)
		{
			cs_set_user_team(iPlayer, HUMAN_TEAM)
		}
