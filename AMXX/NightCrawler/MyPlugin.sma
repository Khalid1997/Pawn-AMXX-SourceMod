...
//#define RANDOM_NIGHTCRAWLERS
	...
	#define CHANGE_NC_EACH_ROUND	// NO CHANGE THIZ
#endif
...
new Trie:g_hTrieMapObjectives, g_hFM_Spawn_Fwd
...
{	
	...
	for(new i; i < sizeof HUMAN_MODELS; i++)
	{
		formatex(szFile, charsmax(szFile), "models/player/%s/%s.mdl", HUMAN_MODELS[i], HUMAN_MODELS[i]);
		precache_model(szFile);
	}
	...
	g_hTrieMapObjectives = TrieCreate()
	for(new i; i < sizeof(szMapObjectives); i++)
	{
		TrieSetCell(g_hTrieMapObjectives, szMapObjectives[i], 1);
	}
	
	g_hFM_Spawn_Fwd = register_forward(FM_Spawn, "fw_EntSpawn", 0);
...
public fw_EntSpawn(iEnt)
{
	...
	if(TrieKeyExists(g_hTrieMapObjectives, szClassName))
	{
		remove_entity(iEnt);
	}
}
...
{	
	...
	unregister_forward(FM_Spawn, g_hFM_Spawn_Fwd, 0);
	TrieDestroy(g_hTrieMapObjectives);
	...
	gMsgIdWeaponPickUp = get_user_msgid("WeapPickup");
...
public client_disconnect(id)
{
	...
	for(new i = TASK_HUD; i < TASKS; i+= 32)
	{
		remove_task(i + id);
	}
	
	set_task(0.1, "CheckAfterDelay");
}

public CheckAfterDelay()
{
	new iPlayers[32], iNum;
	get_players(iPlayers, iNum)
	if(iNum == 1)
	{
		g_iGameState = GS_STOP;
	}
...
public eNewRound()
{
	...
	new iPlayers[32], iNum, iTaskId, iPlayer
	#if defined RANDOM_NIGHTCRAWLERS
	new iTeam
	#endif
	new szName[32];
	...
	if(!g_iGameState)
	{
		for(new i, a; i < iNum; i++)
		{
			iPlayer = iPlayers[i];
			for(a = TASK_HUD; a < TASKS; a += 32)
			{
				iTaskId = a + iPlayer
			
				server_print("a = %d .. iTaskId = %d", a, iTaskId);
				if(task_exists(iTaskId))
				{
					server_print("Remove TASK %d", iTaskId);
					remove_task(iTaskId)
				}
			}
		}
	...
	for(new i, a; i < iNum; i++)
	{
		...
		if( ( iTeam ) == NC_TEAM)
		{
			server_print("Transferring %s from NC to humans, g_iWasNc[iPlayer] = 1", szName)
			cs_set_user_team(iPlayer, HUMAN_TEAM)
			
			g_iWasNc[iPlayer] = 1
		}
	
		// One round passed since leaving nc team
		else if(iTeam == HUMAN_TEAM && g_iWasNc[iPlayer])
		{
			server_print("%s is no longer nc, g_iWasNc[iPlayer] = 0", szName)
			g_iWasNc[iPlayer] = 0
		}
		#else
...
public message_TextMsg(msgid, dest, id)
{
	...
	{
		set_dhudmessage(0, 0, 255, -1.0, 0.20, 0, 0.0, 6.0, 0.1, 0.1)
		show_dhudmessage(0, g_szNewHumanWinMessage)
	...
	{
		set_dhudmessage(255, 0, 0, -1.0, 0.20, 0, 0.0, 6.0, 0.1, 0.1)
		show_dhudmessage(0, g_szNewNcWinMessage)
...
------------------------------------------------------------------------------------------------------ */
public fw_TouchWeapon(iTouched, iToucher)
{
	if(cs_get_user_team(iToucher) == NC_TEAM)
	{
		return PLUGIN_HANDLED;
	}
...
public fw_PlayerSpawn(id)
{
	...
	switch(cs_get_user_team(id))
	{
		...
		case HUMAN_TEAM:
		{
			...
			cs_set_user_model(id, HUMAN_MODELS[random_num(0, charsmax(HUMAN_MODELS))])
	...
	if(pev(id, pev_impulse) == 100)
	{
		...
		else if( iTeam == HUMAN_TEAM )		// EXPLOSION
		{
			...
			else if( IsInBit(id, gHasAdrenaline) )
			{
				...
				set_dhudmessage(187, 187, 187, 10.0, 65.0, 0, 0.0, 6.0, 0.1, 0.1);
				show_dhudmessage(id, "Adrenaline Injected!!")
...
public GiveItemAdminMenuHandler(id, menu, item)
{
	...
	switch(g_iSelectedItem[id])
	{
		...
		case FIRE:
		{
			if(task_exists(id + TASK_NEW_GREN))
			{
				return;
			}
			...
			g_iCounter[id] = COUNTDOWN_GRENADE
			ReciveNewGren(id + TASK_NEW_GREN)
			set_task(1.0, "ReciveNewGren", id + TASK_NEW_GREN,_,_, "b");
		}
...
public ReciveNewGren(id)
{
	id -= TASK_NEW_GREN;
	
	if(g_iCounter[id])
	{
		set_hudmessage(255, 0, 0, 0.0, 0.10, 0, 0.0, 0.9, 0.0, 0.1, -1);
		show_hudmessage(id, "You will recive an extra grenade in %d second", g_iCounter[id]);
	...
	else
	{
		...
		else if( iAmmo < MAX_GRENADES )
		{
			cs_set_user_bpammo(id, CSW_HEGRENADE, iAmmo + 1);
			
			client_cmd(id, "spk ^"sound/items/gunpickup2.wav^"");
			
			message_begin(MSG_ONE, gMsgIdWeaponPickUp, .player = id);
...
public ShowHud(TaskId)
{
	...
	set_hudmessage(0, 100, 200, -1.0, 0.75, 0, 0.0, 0.6, 0.0, 0.0)
	for(i = 0; i < iNum; i++)
	{
		iPlayer = iPlayers[i]
		if(iPlayer == id || ( pev(iPlayer, pev_iuser2) == id && pev(iPlayer, pev_iuser1) == 4 ) )
		{
			show_hudmessage(iPlayer, "Health: %d          Armor: %d^nMana: %d/%d", get_user_health(id), get_user_armor(id), g_iUserMana[id][Mana_AMOUNT], CVARS[Mana])
		}
	}
...
stock ChooseNCPlayers(iPlayers[32], iNum, Slay = 0)
{
	...
	new n, iPlayer, numplayers, iHasChecked, szName[32]
	...
	while(n != iTNum && numplayers != iNum)
	{
		...
		if(g_iWasNc[iPlayer])
		{
			server_print("%s was NC, continue", szName)
			continue;
		}
		
		g_iNcNextRound[iPlayer] = 1
		server_print("%s will be NC", szName)
	...
	if(n != iTNum)
	{
		server_print("Still not completed")
		iHasChecked = 0
		while(n != iTNum)
		{
			iPlayer = iPlayers[random(iNum)]
...
stock ChooseNCPlayers(iPlayers[32], iNum, Slay = 0)
{
	...
	for(i = 0; i < iNum; i++)
	{
		...
		if(g_iNcNextRound[iPlayer])
		{
			...
			/*if(is_user_connected(g_iNcNextRound[iPlayer]) && cs_get_user_team(g_iNcNextRound[iPlayer]) == NC_TEAM)
			{
				cs_set_user_team(g_iNcNextRound[iPlayer], HUMAN_TEAM)
...
{    
	...
	new iFogEnt = create_entity("env_fog");
	
	new szValue[10]
	formatex(szValue, charsmax(szValue), "%f", density_f);
	DispatchKeyValue(iFogEnt, "density", szValue);
	formatex(szValue, charsmax(szValue), "190 190 190", red, green, blue);
	DispatchKeyValue(iFogEnt, "rendercolor", szValue);
}
...
stock GetHighestScore(iIndexes[32], &iEqualTopNum)
{
	...
	for ( new i = 1 ; i < g_iMaxPlayers ; i++ )
	{
		...
		{
			iUserFrags = get_user_frags( i );
			
			iFrags[ i - 1 ][ 0 ] = i;
			iFrags[ i - 1 ][ 1 ] = iUserFrags;
			iNum ++
		}
	}
	...
	server_print("iEqualTopNum = %d", iEqualTopNum);
}
