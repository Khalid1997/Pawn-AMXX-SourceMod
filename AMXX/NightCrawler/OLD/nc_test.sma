...
#define MAXWEP		(CSW_P90 + 1)
new const MAXCLIP[MAXWEP] = { -1, 13, -1, 10, 1, 7, -1, 30, 30, 1, 30, 20, 25, 30, 35, 25, 12, 20,
			10, 30, 100, 8, 30, 30, 20, 2, 7, 30, 30, -1, 50 }
			
// Max BP ammo for weapons
new const MAXBPAMMO[MAXWEP] = { -1, 52, -1, 90, 1, 32, 1, 100, 90, 1, 120, 100, 100, 90, 90, 90, 100, 120,
			30, 120, 200, 32, 90, 120, 90, 2, 35, 90, 90, -1, 100 }
			
// Weapon IDs for ammo types
new const AMMOWEAPON[MAXWEP] = { 0, CSW_AWP, CSW_SCOUT, CSW_M249, CSW_AUG, CSW_XM1014, CSW_MAC10, CSW_FIVESEVEN, CSW_DEAGLE,
			CSW_P228, CSW_ELITE, CSW_FLASHBANG, CSW_HEGRENADE, CSW_SMOKEGRENADE, CSW_C4 }
			
// Ammo Type Names for weapons
new const AMMOTYPE[MAXWEP][] = { "", "357sig", "", "762nato", "", "buckshot", "", "45acp", "556nato", "", "9mm", "57mm", "45acp",
			"556nato", "556nato", "556nato", "45acp", "9mm", "338magnum", "9mm", "556natobox", "buckshot",
			"556nato", "9mm", "762nato", "", "50ae", "556nato", "762nato", "", "57mm" }
			
new const g_szWeaponClassnames[MAXWEP][ ] =
{
...
new const WEPSNAMES[MAXWEP][ ] =
{
	"", "P228", "", "Schmidt Scout", "", "XM1014 (Auto-Shotgun)", "", "Mac-10", "AUG", "", "Dual Elites", "Five-Seven", "UMP-45",
	"SG-550", "Galil", "Famas", "USP", "Glock-18", "AWP", "MP5-Navy", "M249 (Para)", "M3 (Pump-Shotgun)", "M4A1", "TMP", "G3SG1",
	"", "Deagle", "SG-552", "AK-47", "", "P90"
};
...
{	
	...
	register_clcmd("say guns", "
")
	register_clcmd("say /guns", "CmdShowGunsMenu")
	register_clcmd("say_team guns", "CmdShowGunsMenu")
	register_clcmd("say_team /guns", "CmdShowGunsMenu")
...
BuildWepsMenu(id)
{
	gWepsMenu = menu_create("\rChoose weapons", "WeaponsHandler")
	
	//new iItem[50],
	new szInfo[5]
	for(new i; i < MAXWEP; i++)
	{
		if(WEPSNAMES[i][0])
		{
			//formatex(iItem, charsmax(iItem), "
			num_to_str(i, szInfo, charsmax(szInfo))
			menu_additem(gWepsMenu, WEPSNAMES, szInfo)
		}
	}
}

public CmdShowGunsMenu(id)
{
	if(HasChoosed[id])
		return PLUGIN_HANDLED
		
	menu_display(id, gWepsMenu)
	return PLUGIN_CONTINUE
}

public WeaponsHandler(id, menu, item)
{
	new szInfo[5], access, callback
...
{
	if(IsPlayer(id) && cs_get_user_team(id) == NC_TEAM && !IsPlayer(world) )
	{
