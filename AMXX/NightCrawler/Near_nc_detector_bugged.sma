...
public ShowHud(taskid)
{
	...
	// Nearest player
	while( (iEnt = engfunc(EngFunc_FindEntityInSphere, iEnt, fOrigin, MAX_RADIUS ) ) )
	{
		...
		distance_rate(id, fOrigin, iEnt, iColor)
		iFound = 1
		break;
	}
	
	set_hudmessage(iColor[0], iColor[1], iColor[2], -1.0, 0.60, 0, 0.0, UPDATE_TIME - 0.1, 0.0, 0.1)
	show_hudmessage(id, iFound ? g_szPlayerMessage[id] : g_szDefaultMessage)
}

stock distance_rate(id, Float:Origin1[3], iEnt, iColor[3])
{
	static Float:Origin2[3]; pev(iEnt, pev_origin, Origin2)
