...
public plugin_init()
{
	...
	// Edit
	RegisterHam(Ham_CS_Player_ResetMaxSpeed, "player", "HamCallback_ResetMaxSpeed", 1);
...
public HamCallback_ResetMaxSpeed(id)
{
	if(g_flPlayerSpeed[id] >= 0.0)
	{
		set_pev(id, pev_maxspeed, g_flPlayerSpeed[id]);
	}
}
...
public start_game_now()
{
	...
	for(new i = 0; i < g_MaxPlayers; i++)
	{
		if(is_user_connected(i) && is_user_alive(i) && !g_zombie[i])
		{
			...
			//g_flPlayerSpeed[i] = cfg_human_speed;
			set_client_maxspeed(i, cfg_human_speed);
...
public release_zombie()
{
	...
	for(new i = 0; i < g_MaxPlayers; i++)
	{
		...
		// Edit
		///g_flPlayerSpeed[i] = cfg_zombie_speed;
		set_client_maxspeed(i, cfg_zombie_speed);
		server_print("cfg_zombie_speed %0.3f", cfg_zombie_speed);	
...
public set_user_zombie(id, zombie_type, forward_exec)
{
	...
	if(zombie_type == ZOMBIE_TYPE_HOST)
	{
		...
		set_client_maxspeed(id, cfg_zombie_speed);
		server_print("cfg_zombie_speed: %0.2f", cfg_zombie_speed);
		set_user_gravity(id, cfg_zombie_gravity)
	} else {
		if(!forward_exec)
			set_client_maxspeed(id, 0.1);
		else
			set_client_maxspeed(id, cfg_zombie_speed);
		set_user_gravity(id, cfg_zombie_gravity)		
	}
	...
	if(!zombie_type) // Host
	{
		...
		// edit;
		fm_set_user_model(id, temp_string, false)
...
public set_human_stuff(id)
{
	...
	if(g_gamestart < 1)
	{
		...
		set_client_maxspeed(id, cfg_human_speed);
	}
...
public do_set_human_model(id)
{
	...
	fm_set_user_model(id, model, false)	
}
...
// Team: 1 - T | 2 - CT
{
	...
	{
		// edit
		//TerminateRound(EndTime, WINSTATUS_DRAW)
...
set_client_maxspeed(id, Float:maxspeed)
{
	set_pev(id, pev_maxspeed, maxspeed);
	g_flPlayerSpeed[id] = maxspeed;
}
