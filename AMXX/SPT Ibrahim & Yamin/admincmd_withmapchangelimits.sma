...
// Edited for Yamin by GordonFreeman
...
new Handle:g_tuple,g_query[512]

new times[33],acc[33],day

#define TSILVER 3	// how many times silver player can change map
#define SILVER 1
#define GOLD 2
...
public client_disconnect(id)
{
	...
	formatex(g_query,511,"UPDATE map_changing_times SET times='%d',day='%d' WHERE name='%s'",times[id],day,name)
	SQL_ThreadQuery(g_tuple,"query",g_query)
	}
}
public client_putinserver(id){
	...
	if(get_user_flags(id) & ADMIN_LEVEL_G){
		...
		formatex(g_query,511,"SELECT times FROM map_changing_times WHERE name='%s'",name)
		SQL_ThreadQuery(g_tuple,"loadtimes",g_query,Data,1)
...
public plugin_cfg()
{
	...
	formatex(g_query,511,"CREATE TABLE IF NOT EXISTS `map_changing` (name VARCHAR(32), level VARCHAR(10), map VARCHAR(30), date TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)")
	SQL_ThreadQuery(g_tuple,"query",g_query)
	formatex(g_query,511,"CREATE TABLE IF NOT EXISTS `map_changing_times` (`name` VARCHAR(32),`times` INT NOT NULL DEFAULT '0', `day` INT)")
	SQL_ThreadQuery(g_tuple,"query",g_query)
	formatex(g_query,511,"DELETE FROM `map_changing_times` WHERE day!='%d'",day)
	SQL_ThreadQuery(g_tuple,"query",g_query)
}
...
public cmdMap(id, level, cid)
{
	...
	if(!all){
		console_print(id,"[AMXX] You have reached limit for changing map")
		return PLUGIN_HANDLED
	}
	...
	if(all)
		client_print(0,print_chat,"*** %s Player Action: Map will be changed to %s by %s Player: %s",grant[acc[id]],arg,grant[acc[id]],name)
	else
	
	show_activity_key("ADMIN_MAP_1", "ADMIN_MAP_2", name, arg);
	
	log_amx("Cmd: ^"%s<%d><%s><>^" changelevel ^"%s^"", name, get_user_userid(id), authid, arg)
	logon(name,arg,acc[id])
...
public logon(name[],map[],level){
	if(!level)
		return PLUGIN_CONTINUE
		
	new grant[10]
	switch(level){
		case 1: formatex(grant,9,"silver")
		case 2: formatex(grant,9,"golden")
	}
	
	formatex(g_query,511,"INSERT INTO `map_changing` (name,level,map) VALUES('%s','%s','%s')",name,grant,map)
	SQL_ThreadQuery(g_tuple,"query",g_query)
...
allow_change(id){
	if(acc[id]==SILVER&&times[id]<TSILVER)
		return 1
	if(acc[id]==GOLD&&!(get_gametime()<600.0))
		return 1
	if(get_user_flags(id)&ADMIN_RCON)
		return 2
...
public loadtimes(FailState, Handle:Query, Error[], Errcode,Data[],DataSize){
	...
	if(!SQL_MoreResults(Query)){
		...
		formatex(g_query,511,"INSERT INTO `map_changing_times` (name,day) VALUES('%s','%d')",name,day)
		SQL_ThreadQuery(g_tuple,"query",g_query)
