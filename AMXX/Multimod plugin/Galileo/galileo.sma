...
{
	new thisMap[32], mapCnt = ArraySize(mapArray), nextmapIdx = /* 0 */ -1, returnVal = -1;
...
public vote_display(arg[3])
{
	...
	if (g_refreshVoteStatus || isVoteOver)
	{
		...
		new bool:allowExtend = false; //!allowStay && ((isRunoff && g_choiceCnt == 1) || (!(g_voteStatus & VOTE_FORCED) && !isRunoff && get_cvar_float("mp_timelimit") < get_pcvar_float(cvar_extendmapMax)));
		if (get_cvar_num("gal_debug") & 4)
		{
