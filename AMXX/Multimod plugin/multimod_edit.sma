...
new bool:g_bChangeMapNow = false;
...
new const MENU_PREFIX[] = "\r[UaE-Gaming]";
new g_menuname[] = "\wChoose the next mod:"
new g_pVoteTime, g_pRunOffVoteTime, g_pRunOffVote
...
{ 
	new szMenuName[66];
	formatex(szMenuName, charsmax(szMenuName), "%s %s", MENU_PREFIX, g_menuname);
	gMenu = menu_create(szMenuName, "player_vote")
	#if defined SHOW_VOTE_PERCENT
		#if defined SHOW_PERCENT_AFTER_CHOOSING
		gPercentMenu = menu_create(szMenuName, "menu_handler_dump")
		#else
	...
	{	
		...
		{
			#if defined SHOW_VOTE_PERCENT
			formatex(szPercentModName, charsmax(szPercentModName), "%s (0 Votes)", g_modnames[i])
				#if defined SHOW_PERCENT_AFTER_CHOOSING
				...
				#endif
			#else
			menu_additem(gMenu, g_modnames[i], szInfo)
			#endif
		}
	}
	
	#if defined ADD_EXTEND
	menu_setprop(gMenu, MPROP_EXITNAME, "Extend current MOD")
		#if defined SHOW_PERCENT_AFTER_CHOOSING
		menu_setprop(gMenu, MPROP_EXITNAME, "Extend current MOD (0 Votes)")
		#endif
	#endif
	
	new iMenuCallBack = menu_makecallback("MainMenuCallBack")
	
	formatex(szMenuName, charsmax(szMenuName), "%s \wMultimod Menu^nBy Khalid :)", MENU_PREFIX);
	g_iMainMM_Menu = menu_create(szMenuName, "MainMenuHandler")
	...
	formatex(szMenuName, charsmax(szMenuName), "%s \wChoose the next mod:", MENU_PREFIX);
	g_iChooseNextModMenu = menu_create(szMenuName, "ChooseNextModHandler")
	
	formatex(szMenuName, charsmax(szMenuName), "%s \wChoose a mod to Block/Unblock", MENU_PREFIX);
	g_iBlockModMenu = menu_create(szMenuName, "BlockModMenu")
...
public plugin_init()
{
	...
	g_pMins = register_cvar("mm_mins_for_rtmv", "0")
	g_pRTMV = register_cvar("mm_allow_rtmv", "1")
	g_pPercent = register_cvar("mm_rtmv_percent", "75")
	g_pVoteTime = register_cvar("mm_vote_time", "10");
	g_pRunOffVoteTime = register_cvar("mm_runoff_vote_time", "15");
	g_pMM_Menu = register_cvar("mm_allow_menu_access", "1")
	g_pRunOffVote = register_cvar("mm_allow_runoff_vote", "0")
...
{	
	...
	#if defined COLORED
	ColorChat(0, BLUE, "%s ^1ADMIN ^3%s: ^4Start mod vote!", PREFIX, szAdminName)
	#else
...
public start_vote()
{
	...
	formatex(szTitle, charsmax(szTitle), "%s \w%s^n\wTime Remaining: \y%d", MENU_PREFIX, g_menuname, g_iVoteTime)
	menu_setprop(gMenu, MPROP_TITLE, szTitle)
	
	new iPlayers[32], iNum
	get_players(iPlayers, iNum, "ch")
	
	arrayset(g_iHasVoted, sizeof g_iHasVoted, 0);
	
	for(new i/*, iMenu1, iMenu2*/; i < iNum; i++)
	{
		//menu_display(iPlayers[i], gMenu)
		//player_menu_info(iPlayers[i], iMenu1, iMenu2);
		menu_cancel(iPlayers[i]);
		menu_display(iPlayers[i], gMenu);
	}
	
	VoteTimeDecrementInMenu();
...
{
	g_iVoteTime--
	static szMenuName[65]
	formatex(szMenuName, charsmax(szMenuName), "%s %s^n\wTime Remaining: \y%d", MENU_PREFIX, g_menuname, g_iVoteTime)
	...
	for(new i; i < iNum; i++)
	{
		...
		#if defined SHOW_PERCENT_AFTER_CHOOSING
		if( (!g_iHasVoted[iPlayer] && (iMenu != gMenu && iMenu != gPercentMenu) ) )
		#else
		if( (!g_iHasVoted[iPlayer] && iMenu != gMenu)  )
		#endif
		{
			//server_print("displayed menu to %d", iPlayer);
			menu_display(iPlayer, gMenu);
		}
		
		#if defined SHOW_PERCENT_AFTER_CHOOSING
		else if(iMenu == gPercentMenu || iMenu == gMenu)
		#else
		else if(iMenu == gMenu)
		#endif
		{
			menu_display(iPlayer, gMenu, iPage);
		}
	}
}		
...
public user_nextmod(id)
{
	...
	#if defined COLORED
		ColorChat(id, GREEN, "No mod has been chosen yet.")
	#else
		client_print(id, print_chat, "No mod has been chosen yet.")
	#endif
...
{
	#if defined COLORED
	ColorChat(id, BLUE, "%s ^4The current mod is: %s", PREFIX, g_modnames[g_currentmodid])
	#else
...
public menu_voteCallBack(id, menu, item)
{
	...
	if(!g_iHasVoted[id])
	{
		//server_print("Has not Voted")
		return ITEM_ENABLED
	}
	
	//server_print("Disabled")
	return ITEM_DISABLED
}
...
stock UpdateMenuPercent(iItem, iModNum)
{
	...
	formatex(szNewName, charsmax(szNewName), "%s (%d Votes)", g_modnames[iModNum], g_votemodcount[iModNum])
...
public RunOffVote(iParams[])
{
	new szTitle[60];
	new Float:flTime
	formatex(szTitle, charsmax(szTitle), "%s \w%s^nTime remaining: %d", MENU_PREFIX, g_menuname, ( g_iVoteTime = floatround( ( flTime = get_pcvar_float(g_pRunOffVoteTime) ) ) ) )
	...
	for(new i; i < 2; i++)
	{
		...
		#if defined SHOW_VOTE_PERCENT
			formatex(szTitle, charsmax(szTitle), "%s (0 Votes)", g_modnames[iParams[i]])
...
public StartMapVote()
{
	...
	switch(iNum)
	{
		...
		case 1:
		{
			#if defined GALILEO_SUPPORT
			// FORCE VOTE
			callfunc_push_int(_:g_bChangeMapNow)
			#else
			callfunc_push_int(_:g_bChangeMapNow)
			#endif
			callfunc_end();
