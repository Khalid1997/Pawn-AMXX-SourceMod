...
public Function_GiveGrenade(iTaskId)
{
	...
	switch( user_has_weapon( ( id ), CSW_SMOKEGRENADE ) )
	{
		case 1:
		{
			if( ( iPlayerHeGrenades = cs_get_user_bpammo(id, CSW_SMOKEGRENADE ) ) >= MAX_SMOKE_NADES )
			{
				return;
			}
				
			cs_set_user_bpammo(id, CSW_SMOKEGRENADE, iPlayerHeGrenades + 1)
		}
