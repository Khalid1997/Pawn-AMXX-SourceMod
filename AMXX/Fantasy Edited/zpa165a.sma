...
=================================================================================*/
new const ZP_CUSTOMIZATION_FILE[] = "n4d_zpa.ini"
new const ZP_EXTRAITEMS_FILE[] = "n4d_zpa_items.ini"
new const ZP_ZOMBIECLASSES_FILE[] = "n4d_zpa_zclass.ini"
new const ZP_CONFIGURATION_FILE[] = "n4d_zpa.cfg"
new const ZP_MULTILANG_FILE[] = "n4d_zpa.txt"
...
//This module will be use too with other plugins so don't say you are using fakemental only.
#include <fun>
#include <engine>
#include <geoip>
...
/////by wbyokomo
#define SUR_GLOW_R 0
#define SUR_GLOW_G 191
#define SUR_GLOW_B 255

#define SNI_GLOW_R 138
#define SNI_GLOW_G 43
#define SNI_GLOW_B 226

#define NEM_GLOW_R 255
#define NEM_GLOW_G 64
#define NEM_GLOW_B 64

#define ASS_GLOW_R 210
#define ASS_GLOW_G 105
#define ASS_GLOW_B 30

#define MAD_GLOW_R 173
#define MAD_GLOW_G 255
#define MAD_GLOW_B 47
/////////
/*================================================================================
...
// Plugin Version
new const PLUGIN_VERSION[] = "1.6.5p"
...
new const MAXBPAMMO[] = { -1, 999, -1, 999, 1, 999, 1, 999, 999, 1, 999, 999, 999, 999, 999, 999, 999, 999,
			999, 999, 999, 999, 999, 999, 999, 2, 999, 999, 999, -1, 999 }
...
new const BUYAMMO[] = { -1, 999, -1, 999, -1, 999, -1, 999, 999, -1, 999, 999, 999, 999, 999, 999, 999, 999,
			999, 999, 999, 999, 999, 999, 999, -1, 999, 999, 999, -1, 999 }
...
new Float:g_damagedealt_human[33] // damage dealt to zombies (used to calculate ammo packs reward)
new Float:g_damagedealt_zombie[33] // damage dealt as zombie (used to calculate ammo packs reward)
...
cvar_nemchance, cvar_deathmatch, cvar_customnvg, cvar_humanhp,
cvar_nemgravity,
cvar_nemspd, cvar_survchance, cvar_survhp, cvar_survspd, cvar_humanspd,
cvar_swarmchance, cvar_removedoors,
cvar_randspawn, cvar_multi, cvar_multichance, cvar_infammo, cvar_swarm, cvar_ammoinfect,
cvar_freezeduration, cvar_triggered,
cvar_frostgrenades, cvar_survgravity, cvar_logcommands,
...
cvar_extraweapons, cvar_extranvision, cvar_nvggive, cvar_preventconsecutive,
cvar_buycustom, cvar_survbasehp,
cvar_extrainfbomb,
cvar_fragsinfect, cvar_removedropped,
cvar_plagueratio, cvar_blocksuicide, cvar_nemminplayers, cvar_survminplayers,
cvar_respawnafterlast,
cvar_swarmminplayers, cvar_multiminplayers, cvar_plagueminplayers, cvar_adminmodelshuman,
cvar_adminmodelszombie, cvar_nembasehp, cvar_respawnworldspawnkill,
cvar_madnessduration, cvar_plaguenemnum, cvar_plaguenemhpmulti, cvar_plaguesurvhpmulti,
cvar_plaguesurvnum,
cvar_infbomblimit,
cvar_allowrespawnsurv, cvar_allowrespawninfection, cvar_allowrespawnnem,
cvar_allowrespawnswarm, cvar_allowrespawnplague,
cvar_respawnzomb, cvar_respawnhum, cvar_respawnnem, cvar_respawnsurv,
cvar_startammopacks, cvar_randweapons, cvar_antidotelimit, cvar_madnesslimit,
cvar_adminknifemodelshuman, cvar_adminknifemodelszombie,
cvar_sniper, cvar_sniperchance, cvar_sniperminplayers, cvar_sniperhp,
cvar_sniperbasehp, cvar_sniperspd,
cvar_respawnsniper, cvar_snipergravity,
...
cvar_assassinhp , cvar_assassinbasehp ,cvar_assassinspd ,cvar_assassingravity,
cvar_humansurvive,
cvar_lnj, cvar_lnjchance, cvar_lnjminplayers, cvar_lnjnemhpmulti, cvar_lnjsurvhpmulti,  cvar_allowrespawnlnj,
cvar_lnjratio, cvar_lnjrespsurv, cvar_lnjrespnem, cvar_huddisplay
...
public plugin_natives()
{
	...
	//by wbyokomo
	register_native("zp_get_user_frozen", "_get_user_frozen", 1)
	register_native("zp_set_user_frozen", "_set_user_frozen", 1)
	register_native("zp_get_zombie_class_max", "_get_zombie_class_max", 1)
	register_native("zp_get_last_mode", "_get_last_mode", 1)
}

public _get_zombie_class_max()
{
	return g_zclass_i;
}

public _get_user_frozen(id)
{
	return g_frozen[id];
}

public _set_user_frozen(id, set)
{	
	g_frozen[id] = set ? true:false
	ExecuteHamB(Ham_Player_ResetMaxSpeed, id)
}

public _get_last_mode()
{
	return g_lastmode;
}
...
public plugin_precache()
{
	...
	register_plugin("Zombie Plague Advance", PLUGIN_VERSION, "MeRcyLeZZ, @bdul!, 93()|29!/<, wbyokomo")
...
public plugin_init()
{
	...
	server_print("[ZP] Total Registered Game Modes: %d", g_gamemodes_i)
	...
	// Language files
	register_dictionary(ZP_MULTILANG_FILE)
	...
	cvar_warmup = register_cvar("zp_delay", "15")
	cvar_lighting = register_cvar("zp_lighting", "e")
	...
	cvar_customnvg = register_cvar("zp_nvg_custom", "0")
	...
	cvar_flaregrenades = register_cvar("zp_fire_grenades","1")
	...
	// CVARS - Others
	cvar_logcommands = register_cvar("zp_logcommands", "0")
	cvar_showactivity = get_cvar_pointer("amx_show_activity")
	
	//it would be good if you dont change it, please appreciate our job.
	register_cvar("zp_version_adv", PLUGIN_VERSION, FCVAR_SERVER|FCVAR_SPONLY)
	set_cvar_string("zp_version_adv", PLUGIN_VERSION)
	register_cvar("zp_version", PLUGIN_VERSION, FCVAR_SERVER|FCVAR_SPONLY)
	set_cvar_string("zp_version", PLUGIN_VERSION)
	register_cvar("zp_version_n4d", PLUGIN_VERSION, FCVAR_SERVER|FCVAR_SPONLY)
	set_cvar_string("zp_version_n4d", PLUGIN_VERSION)
...
public plugin_cfg()
{
	...
	server_cmd("exec %s/%s", cfgdir, ZP_CONFIGURATION_FILE)
...
public event_round_start()
{
	...
	set_task(1.0 + get_pcvar_float(cvar_warmup), "make_zombie_task", TASK_MAKEZOMBIE)
}
...
public logevent_round_end()
{
	...
	//FIX for assassin round lighting
	if(g_assassinround)
	{
		static lighting[2]
	...
	if (!g_newround)
	{
		...
		if (g_respawn_as_zombie[id])
		{
			...
			if (g_lnjround && get_pcvar_num(cvar_lnjrespnem))
			{
				...
				set_user_health(id, floatround(float(pev(id, pev_health)) * get_pcvar_float(cvar_lnjnemhpmulti)))
			}
		...
		else
		{
			...
			if (g_lnjround && get_pcvar_num(cvar_lnjrespsurv))
			{
				...
				set_user_health(id, floatround(float(pev(id, pev_health)) * get_pcvar_float(cvar_lnjsurvhpmulti)))
			}
		}
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	if (!selfkill)
	{
		...
		//explode them all...
		if (g_sniper[attacker] || g_nemesis[attacker] || g_assassin[attacker])
		{	
			// Cut him into pieces
...
public fw_PlayerKilled_Post(victim, attacker, shouldgib)
{
	...
	static dmmode
	dmmode = get_pcvar_num(cvar_deathmatch)
	if (dmmode || ((g_currentmode > MODE_LNJ) && (g_deathmatchmode > 0)))
	{
		...
		else if (dmmode > 0) // Normal round
		{
			if (dmmode == 2 || (dmmode == 3 && random_num(0, 1)) || (dmmode == 4 && (fnGetZombies() < (fnGetAlive()/2))))
				g_respawn_as_zombie[victim] = true
		}
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if (g_newround || g_endround || (victim == attacker)) return HAM_SUPERCEDE;
		
	// Non-player damage or self damage
	if (!is_user_valid_connected(attacker)) return HAM_IGNORED;
	
	// Victim shouldn't take damage or victim is frozen
	if (get_user_godmode(victim) || g_frozen[victim]) return HAM_SUPERCEDE;
	...
	if (!g_zombie[attacker])
	{
		if (damage_type & DMG_GRENADE)
		{
			...
			zp_colored_print(attacker, "^x04[ZP]^x01 Damage to^x04 %s^x01 ::^x04 %.0f^x01 damage", g_playername[victim], damage)
		}
		...
		if (g_sniper[attacker] && (g_currentweapon[attacker] == CSW_AWP))
		{
			damage = 2222.2
			SetHamParamFloat(4, damage)
			g_damagedealt_human[attacker] += damage
			while (g_damagedealt_human[attacker] > 500.0)
			{
				g_ammopacks[attacker]++
				g_damagedealt_human[attacker] -= 500.0
			}
	...
	if (damage_type & DMG_GRENADE) return HAM_SUPERCEDE;
	...
	if (g_nemesis[attacker] || g_assassin[attacker])
	{
		...
		{
			// Set proper damage
			damage = g_nemesis[attacker] ? 200.0 : 100.0
			SetHamParamFloat(4, damage)
		}
	...
	g_damagedealt_zombie[attacker] += damage
		
	// Reward ammo packs for every [ammo damage] dealt
	while (g_damagedealt_zombie[attacker] > 250.0)
	{
		g_ammopacks[attacker]++
		g_damagedealt_zombie[attacker] -= 250.0
	}
	
	// Last human or infection not allowed
	if (!g_allowinfection || fnGetHumans() == 1) return HAM_IGNORED; // human is killed
...
public fw_TakeDamage_Post(victim)
{
	if(g_zombie[victim])
	{
		if(g_assassin[victim] || g_lastzombie[victim])
		{
			if(pev_valid(victim) == PDATA_SAFE) set_pdata_float(victim, OFFSET_PAINSHOCK, 1.0, OFFSET_LINUX);
			return;
		}
	}
	else
...
public fw_TraceAttack(victim, attacker, Float:damage, Float:direction[3], tracehandle, damage_type)
{
	...
	if (get_distance(origin1, origin2) > 500) return HAM_IGNORED;
	...
	if (g_nemesis[victim]) xs_vec_mul_scalar(direction, 0.3, direction);
	else if (g_assassin[victim]) xs_vec_mul_scalar(direction, 2.0, direction);
	else xs_vec_mul_scalar(direction, g_zombie_knockback[victim], direction);
...
public fw_TouchWeapon(weapon, id)
{
	...
	if (g_zombie[id] || (!g_isbot[id] && (g_survivor[id] || g_sniper[id])))
		return HAM_SUPERCEDE;
...
public client_putinserver(id)
{
	...
	if (!is_user_bot(id))
	{
		...
		if (get_pcvar_num(cvar_huddisplay)) set_task(1.0, "ShowHUD", id+TASK_SHOWHUD, _, _, "b");
		
		new szIP[32]
		get_user_ip(id, szIP, 31, 1)
		if(!geoip_country_ex(szIP, g_country[id], 45)) format(g_country[id], 45, "Strange Country");
		if(!geoip_city(szIP, g_city[id], 31)) format(g_city[id], 31, "Ghost City");
		client_print_color(0, print_team_default, "^4%s^1 connected from^4 %s^1,^4 %s", g_playername[id], g_city[id], g_country[id])
	...
	else
	{
		// Set bot flag
		g_isbot[id] = true
		format(g_country[id], 45, "United Gamer")
		format(g_city[id], 31, "Raccoon City")
	}
}
...
public client_disconnected(id)
{
	...
	client_print_color(0, print_team_default, "^4%s^1 disconnected from^4 %s^1,^4 %s", g_playername[id], g_city[id], g_country[id])
...
public fw_SetModel(entity, const model[])
{
	...
	if (get_pcvar_float(cvar_removedropped) > 0.0)
	{
		...
		if (classname[0]=='w' && classname[8]=='x')
		{
	...
	if (g_zombie[pev(entity, pev_owner)])
	{
		...
		{
			// Give it a glow
			set_ent_rendering(entity, kRenderFxGlowShell, 0, 250, 0, kRenderNormal, 0)
	...
	{
		// Give it a glow
		set_ent_rendering(entity, kRenderFxGlowShell, 200, 0, 0, kRenderNormal, 0)
	...
	{
		// Give it a glow
		set_ent_rendering(entity, kRenderFxGlowShell, 0, 100, 200, kRenderNormal, 0)
	...
	else if (model[9] == 's' && model[10] == 'm' && get_pcvar_num(cvar_flaregrenades)) // Now this is napalm
	{
		// Give it a glow
		set_ent_rendering(entity, kRenderFxGlowShell, 200, 0, 0, kRenderNormal, 0)
...
public fw_CmdStart(id, handle)
{
	...
	if (!g_zombie[id] && !g_survivor[id] && !g_sniper[id] && g_zombie[id])
		return;
...
public show_menu_buy1(taskid)
{
	...
	//Rebuild using new menu
	static menuid, menu[300], weap, buffer[32], maxloops
	maxloops = ArraySize(g_primary_items)
	
	// Title
	formatex(menu, charsmax(menu), "%L\r", id, "MENU_BUY1_TITLE")
	...
	{
		// Add to menu
		formatex(buffer, charsmax(buffer), "%s", WEAPONNAMES[ArrayGetCell(g_primary_weaponids, weap)]) //item name
		formatex(menu, charsmax(menu), "%s", buffer) //item info
		buffer[0] = weap
		buffer[1] = 0
		menu_additem(menuid, menu, buffer)
	}
	
	// No weapon to display?
...
{	
	...
	// Title
	formatex(menu, charsmax(menu), "%L\r", id, "MENU_BUY2_TITLE")
	menuid = menu_create(menu, "menu_buy2")
	
	// Weapon List
	for (weap = 0; weap < maxloops; weap++)
	{
		// Add to menu
		formatex(buffer, charsmax(buffer), "%s", WEAPONNAMES[ArrayGetCell(g_secondary_weaponids, weap)]) //item name
		formatex(menu, charsmax(menu), "%s", buffer) //item info
		buffer[0] = weap
		buffer[1] = 0
		menu_additem(menuid, menu, buffer)
	...
	switch (key)
	{
		case 0: // Buy Weapons
		{
			...
			if (get_pcvar_num(cvar_buycustom))
			{
				...
				else zp_colored_print(id, "^x04[ZP]^x01 Buy menu can only be used once.");
			}
			else
		...
		case 3: // Unstuck
		{
			...
			set_user_unstuck(id) //call external native
		}
		/*case 4: // Help Menu
		{
			//
		...
		case 5: // Join Spectator
		{
			...
			fm_user_team_update(id)
			zp_colored_print(id, "^x04[ZP]^x01 You've been transfered to Spectator team.")
		}
		case 8: // Admin Menu
		{
...
// Buy Menu 1: Rebuild using new menu
public menu_buy1(id, menuid, item)
{
	...
	// Dead players, Zombies, survivors or snipers get no guns
	if (!g_isalive[id] || g_zombie[id] || g_survivor[id] || g_sniper[id])
	{
		menu_destroy(menuid)
		return PLUGIN_HANDLED;
...
// Buy Menu 2: Rebuild using new menu
public menu_buy2(id, menuid, item)
{
	// Player disconnected?
	if (!is_user_connected(id))
	{
	...
	menu_item_getinfo(menuid, item, dummy, buffer, charsmax(buffer), _, _, dummy)
	itemid = buffer[0]
	buy_secondary_weapon(id, itemid)
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	|| (itemid == EXTRA_MADNESS && get_user_godmode(id)) || (itemid == EXTRA_INFBOMB && (g_endround || g_swarmround || g_nemround || g_survround || g_plagueround || g_assassinround || g_sniperround || g_lnjround)))
	{
	...
	switch (itemid)
	{
		...
		case EXTRA_MADNESS: // Zombie Madness
		{
			...
			set_user_godmode(id, 1)
			set_user_rendering(id, kRenderFxGlowShell, MAD_GLOW_R, MAD_GLOW_G, MAD_GLOW_B, kRenderNormal, 250)
			set_task(get_pcvar_float(cvar_madnessduration), "madness_over", id+TASK_BLOOD)
			client_print_color(id, id, "^4[ZP]^1 You are invulnerable for^3 %.0f^1 seconds.", get_pcvar_float(cvar_madnessduration))
			
			static sound[64]
			ArrayGetString(zombie_madness, random_num(0, ArraySize(zombie_madness) - 1), sound, charsmax(sound))
			emit_sound(id, CHAN_STREAM, sound, 1.0, ATTN_NORM, 0, PITCH_NORM)
		}
...
public menu_zclass(id, menuid, item)
{
	...
	static name[32], szHp[14], szSpeed[14]
	ArrayGetString(g_zclass_name, g_zombieclassnext[id], name, charsmax(name))
	AddCommas(ArrayGetCell(g_zclass_hp, g_zombieclassnext[id]), szHp, charsmax(szHp))
	AddCommas(ArrayGetCell(g_zclass_spd, g_zombieclassnext[id]), szSpeed, charsmax(szSpeed))
	
	// Show selected zombie class info and stats
	zp_colored_print(id, "^x04[ZP]^x01 %L:^x04 %s", id, "ZOMBIE_SELECT", name)
	zp_colored_print(id, "^x04[ZP]^x01 %L:^x04 %s^x01 %L:^x04 %s^x01 %L:^x04 %d^x01 %L:^x04 %d%%", id, "ZOMBIE_ATTRIB1", szHp, id, "ZOMBIE_ATTRIB2", szSpeed,
...
public menu4_admin(id, key)
{
	...
	switch (key)
	{
		...
		{
			client_print_color(id, id, "^4[ZP]^1 Tadaaaaa useless menu ever existed.")
		}
		case 1: // Return
		{
			show_menu3_admin(id)
...
zombieme(id, infector, nemesis, silentmode, rewards, assassin)
{
	...
	if (!silentmode)
	{
		if (nemesis)
		{
			...
			if (!g_frozen[id]) set_user_gravity(id, get_pcvar_float(cvar_nemgravity))
		...
		else if (assassin)
		{
			...
			if (get_pcvar_num(cvar_assassinhp) == 0)
			{
				if (get_pcvar_num(cvar_assassinbasehp) == 0)
					...
					set_user_health(id, get_pcvar_num(cvar_assassinbasehp) * fnGetAlive())
			}
			else
				set_user_health(id, get_pcvar_num(cvar_assassinhp))
			
			// Set gravity, unless frozen
			if (!g_frozen[id]) set_user_gravity(id, get_pcvar_float(cvar_assassingravity))
		...
		else if ((fnGetZombies() == 1) && !g_assassin[id] && !g_nemesis[id] )
		{
			...
			if (!g_frozen[id]) set_user_gravity(id, Float:ArrayGetCell(g_zclass_grav, g_zombieclass[id]))
	...
	else
	{
		...
		{				
			if(g_nemesis[id])
			{
				set_user_rendering(id, kRenderFxGlowShell, NEM_GLOW_R, NEM_GLOW_G, NEM_GLOW_B, kRenderNormal, 0)
			}
			else
			{
				set_user_rendering(id)
...
humanme(id, survivor, silentmode, sniper)
{
	...
	if (survivor)
	{
		...
		// Set gravity, unless frozen
		if (!g_frozen[id]) set_user_gravity(id, get_pcvar_float(cvar_survgravity))
		else g_frozen_gravity[id] = get_pcvar_float(cvar_survgravity)
		
		// Set survivor maxspeed
		ExecuteHamB(Ham_Player_ResetMaxSpeed, id)
		
		// Give survivor his own weapon, we better make through external plugin
	...
	else if (sniper)
	{
		...
		if (get_pcvar_num(cvar_sniperhp) == 0)
		{
			if (get_pcvar_num(cvar_sniperbasehp) == 0)
				...
				set_user_health(id, get_pcvar_num(cvar_sniperbasehp) * fnGetAlive())
		}
		else
			set_user_health(id, get_pcvar_num(cvar_sniperhp))
		
		// Set gravity, unless frozen
		if (!g_frozen[id]) set_user_gravity(id, get_pcvar_float(cvar_snipergravity))
	...
	else
	{
		...
		// Set gravity, unless frozen
		if (!g_frozen[id]) set_user_gravity(id, get_pcvar_float(cvar_humangravity))
	...
	else
	{
		...
		{				
			if(g_survivor[id])
			{
				set_user_rendering(id, kRenderFxGlowShell, SUR_GLOW_R, SUR_GLOW_G, SUR_GLOW_B, kRenderNormal, 0)
			}
			else if(g_sniper[id])
			{
				set_user_rendering(id, kRenderFxGlowShell, SNI_GLOW_R, SNI_GLOW_G, SNI_GLOW_B, kRenderNormal, 0)
			}
			else
			{
				set_user_rendering(id)
...
public respawn_player_task(taskid)
{
	...
	&& (!g_nemround || get_pcvar_num(cvar_allowrespawnnem)) && (!g_plagueround || get_pcvar_num(cvar_allowrespawnplague))
	&& (!g_sniperround || get_pcvar_num(cvar_allowrespawnsniper)) && (!g_assassinround || get_pcvar_num(cvar_allowrespawnassassin))
	&& (!g_lnjround || get_pcvar_num(cvar_allowrespawnlnj)) && (!(g_currentmode > MODE_LNJ) || (g_deathmatchmode > 0)))
	{
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		client_print_color(0, print_team_grey, "^4[ZP]^1 Last zombie^3 %s^1 has left,^3 %s^1 become the new zombie.", g_playername[leaving_player], g_playername[id])
		...
		//Why for nemesis/assassin only? make for zombie too
	...
	else if (!g_zombie[leaving_player] && fnGetHumans() == 1)
	{
		...
		client_print_color(0, print_team_grey, "^4[ZP]^1 Last human^3 %s^1 has left,^3 %s^1 become the new human.", g_playername[leaving_player], g_playername[id])
		...
		//Why for survivor/sniper only? make for human too
		set_user_health(id, pev(leaving_player, pev_health))
	}
}
...
infection_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		if (!is_user_valid_alive(victim) || g_zombie[victim] || get_user_godmode(victim))
			continue;
		
		if (g_survivor[victim] || g_sniper[victim])
		{
			ExecuteHamB(Ham_TakeDamage, victim, ent, attacker, 100.0, DMG_BULLET)
			continue;
		}
		...
		if (fnGetHumans() == 1)
		{
			ExecuteHamB(Ham_TakeDamage, victim, ent, attacker, pev(victim, pev_health)+2.0, DMG_BULLET)
			continue;
		}
...
fire_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		{
			ExecuteHamB(Ham_TakeDamage, victim, ent, attacker, random_float(100.0,300.0), DMG_GRENADE)
		}
		else
		{
			...
			if (!task_exists(victim+TASK_BURN))
			{
				new params[1]
				params[0] = attacker
				set_task(0.2, "burning_flame", victim+TASK_BURN, params, 1, "b")				
				set_dhudmessage(200, 32, 32, -1.0, 0.60, 0, 6.0, get_pcvar_float(cvar_fireduration), 0.1, 0.2)
				show_dhudmessage(victim, "Arghh! Fire! Fire! Fire!")
			}
		}
	}
...
frost_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		if (!is_user_valid_alive(victim) || !g_zombie[victim] || g_frozen[victim] || get_user_godmode(victim))
			continue;
		...
		set_user_rendering(victim, kRenderFxGlowShell, 0, 100, 200, kRenderNormal, 250)
		...
		g_frozen_gravity[victim] = get_user_gravity(victim)
...
reset_vars(id, resetall)
{
	...
	if (resetall)
	{
		...
		g_damagedealt_human[id] = 0.0
		g_damagedealt_zombie[id] = 0.0
	}
}
...
public ShowHUD(taskid)
{
	...
	// Format classname
	static class[32], szHp[14], szArmor[14], szAp[14], red, green, blue
	...
	if (id != ID_SHOWHUD)
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "Spectating: %s | Origin: %s, %s^nClass: %s | Health: %s | Armor: %s | Packs: %s", g_playername[id], g_city[id], g_country[id], class, szHp, szArmor, szAp)
	}
	else
	{
		...
		set_hudmessage(red, green, blue, HUD_STATS_X, HUD_STATS_Y, 0, 6.0, 1.1, 0.6, 0.5, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "Origin: %s, %s^nClass: %s | Health: %s | Armor: %s | Packs: %s", g_city[id], g_country[id], class, szHp, szArmor, szAp)
	}
}
...
public madness_over(taskid)
{
	new id=ID_BLOOD
	set_user_godmode(id, 0)
	client_print_color(id, id, "^4[ZP]^1 You no longer invulnerable.")
...
public native_infect_user(id, infector, silent, rewards)
{
	...
	{
		log_error(AMX_ERR_NATIVE, "[ZPA] Invalid Player (%d)", id)
		return false;
	}
...
public native_get_zombie_class_info(classid, info[], len)
{
	...
	if (classid < 0 || classid >= g_zclass_i)
	{
		log_error(AMX_ERR_NATIVE, "[ZPA] Invalid zombie class id (%d)", classid)
		return false;
	}
...
public set_user_nvision(taskid)
{
	...
	if (g_nemesis[ID_NVISION] || (!g_isalive[ID_NVISION] && g_nemround))
	{
		write_byte(NEM_GLOW_R) // r
		write_byte(NEM_GLOW_G) // g
		write_byte(NEM_GLOW_B) // b
	}
	// Assassin / Spectator in assassin round
	else if (g_assassin[ID_NVISION] || (!g_isalive[ID_NVISION] && g_assassinround))
	{
		write_byte(ASS_GLOW_R) // r
		write_byte(ASS_GLOW_G) // g
		write_byte(ASS_GLOW_B) // b
	}
	// Human / Spectator in normal round
	else if (!g_zombie[ID_NVISION] || !g_isalive[ID_NVISION])
	{
		write_byte(SUR_GLOW_R) // r
		write_byte(SUR_GLOW_G) // g
		write_byte(SUR_GLOW_B) // b
	}
	// Zombie
	else
	{
		write_byte(MAD_GLOW_R) // r
		write_byte(MAD_GLOW_G) // g
		write_byte(MAD_GLOW_B) // b
	}
...
// Burning Flames
public burning_flame(params[], taskid)
{
	//security check
	if(!is_user_connected(ID_BURN))
	{
	...
	if (get_user_godmode(ID_BURN) || (flags & FL_INWATER) || g_burning_duration[ID_BURN] < 1)
	{
		// Smoke sprite
		message_begin(MSG_PVS, SVC_TEMPENTITY, origin)
	...
	if (!random_num(0, 20))
	{
		...
		emit_sound(ID_BURN, CHAN_STATIC, sound, 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
	...
	if(health > 30)
	{
		set_user_health(ID_BURN, health)
	}
	else
	{
		...
		if(is_valid_ent(ent))
		{
			entity_set_string(ent, EV_SZ_classname, "n4d_napalm")
			new attacker = params[0]
			if(is_user_connected(attacker)) ExecuteHamB(Ham_TakeDamage, ID_BURN, ent, attacker, 35.0, DMG_BULLET);
			else ExecuteHamB(Ham_TakeDamage, ID_BURN, ent, ent, 35.0, DMG_BULLET);
			remove_entity(ent)
		}
...
// Collect spawn points from entity origins
collect_spawns_ent2(const classname[])
{
	new ent = -1
	while ((ent = engfunc(EngFunc_FindEntityByString, ent, "classname", classname)) != 0)
	{
...
is_hull_vacant(Float:origin[3], hull)
{
...
public fm_cs_set_user_team_msg(taskid)
{
	...
	//FIX: PODBot & YaPB team detection
	message_begin(MSG_ALL, g_msgScoreInfo)
	write_byte(ID_TEAM) // id
	write_short(pev(ID_TEAM, pev_frags)) // frags
	write_short(cs_get_user_deaths(ID_TEAM)) // deaths
	write_short(0) // class?
	write_short(fm_cs_get_user_team(ID_TEAM)) // team
	message_end()
...
fm_cs_get_user_model(player, model[], len)
{
