...
public AdminCmdSetTime(id, level, cid)
{
	...
	if( ( iArgs = read_argc() ) == 3)
	{
		...
		else	set_user_played_time(iPlayer, iTime)
	}
	...
	client_print(0, print_chat, "ADMIN %s: %s %s%s total time%s %d", szName, ( iArgs == 3 ? "Set" : ( szMark[0] == '+' ? "Add to" : "Take from") ), ( !iTeam ? szName : ( iTeam == 3 ? "Everyone's" : ( iTeam == 2 ? "CT team players" : "TERRORIST team players" ) ) ), iTeam ? "" : "'s", iArgs == 3 ? " to" : "", iTime / 60)
	return PLUGIN_HANDLED
}
