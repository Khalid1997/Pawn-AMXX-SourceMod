...
new const g_szPrefix[] =			"^4[KFC]";
#if defined INCLUDE_BOT
	new const g_szBotName[] =		"KFC BOT";
#endif
...
/*
#if MIN_PLAYERS <= 1
	#assert " What are you thinking? Minimum players can't be 1 or less !"
#endif*/
...
public plugin_init( )
{
	register_plugin( "Chicken Mod", "1.0", "Yousef & Khalid" );
...
public fw_TakeDamage_Pre(victim, idinflictor, attacker, Float:damage, damagebits)
{
	...
	if( victim != attacker )
	{
		new Float:Health; 
		new iDump;
		if(get_user_weapon(attacker, iDump, iDump) != CSW_KNIFE)
		{
		...
		if(fm_cs_get_user_team(victim) == fm_cs_get_user_team(attacker) )
		{
			...
			ExecuteHamB(Ham_TakeDamage, victim, idinflictor, attacker, Health, damagebits);
			fm_cs_set_user_team(victim, 2);
			return HAM_SUPERCEDE;
		}
		
		SetHamParamFloat(4, Health);
	}
	return HAM_IGNORED;
}
...
public fw_PlayerTouchGun(iEnt, iPlayer)
{
	...
	if(!g_bWeaponFound && g_iEnt == iEnt)
	{
		...
		if(iEntry + 1 > sizeof(gGunsInfo))
		{
			...
			DestroyForward(g_hWeaponTouchForward);
		}
...
public ShopHandler(id, menu, item)
{
	...
	else
	{
		...
		DestroyForward(g_hShopChoosedForward);
...
stock IsEntStuck(iEnt)
{
	...
	for(new i, j; i < 2; i++)
	{
		...
		for(j = 0; j < 3; j++)
		{
			...
			server_print("Check %0.2f", flPosition[j]);
...
stock CreateTestEntity(iEntry, Float:vOrigin[3])
{
	...
	new Float:Gravity;
	pev(iEnt, pev_gravity, Gravity);
	server_print("Gravity %f", Gravity);
...
stock GetHitAimStuff(id, &iHitEnt, Float:vEndPoint[3] = { 0.0, 0.0, 0.0 }, iAddPlane = 1)
{
	...
	if(flFraction == 1.0)
	{
		console_print(0, "No solid place in sight. IMPOSSIBLE!!");
		return 0;
	}
