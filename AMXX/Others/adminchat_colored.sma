...
public cmdSayAdmin(id)
{
	...
	new iFlags
	new szPrefix[50] = ( is_user_admin(id) ? ( ( iFlags = get_user_flags(id) ) & ADMIN_LEVEL_G ? "SILVER" : ( iFlags & ADMIN_LEVEL_H ? "GOLDEN" : "ADMIN" ) ) : "PLAYER" );
	new Color:iColor = iIsAdmin ? GREEN : RED
	
	format(message, 191, "(%L) %s :  %s", id, iIsAdmin ? get_user_flags(id) & "ADMIN" : "PLAYER", name, message[1])
	...
	for (new i = 0; i < inum; ++i)
	{
		...
		if (players[i] != id && get_user_flags(players[i]) & g_AdminChatFlag)
			...
			ColorChat(players[i], iColor, message)
	}
	...
	else ColorChat(id, BLUE, message)
...
public cmdPsay(id, level, cid)
{
	...
	if (id && id != priv)
		...
		ColorChat(id, GREEN, "You PM ^3%s: %s", name, message[length])
	
	//client_print(priv, print_chat, "(%s) %s :   %s", name, name2, message[length])
	ColorChat(priv, NORMAL, "%s ^4PMs You: %s", name2, message[length])
