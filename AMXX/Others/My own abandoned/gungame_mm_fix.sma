...
win(winner,loser)
{
	...
	new Float:flTime = floatmax(chattime, 2.5)
	// final playthrough, get ready for next map
	if(mapIteration >= map_iterations && map_iterations > 0)
	{
		set_nextmap();
		set_task(flTime, "goto_nextmap");
		gungame_print(0, 0, 1, "Starting votemod in %d seconds!", floatround(flTime))
...
stock teamplay_update_level(team,newLevel,exclude=0,direct=1)
{
	...
	for(player=1;player<=maxPlayers;player++)
	{
		if(is_user_connected(player) && player != exclude && _:cs_get_user_team(player) == team)
		{
			...
			{
				change_level(player,newLevel-level[player],_,_,1); // always score
			}
...
public goto_nextmap()
{
	...
	*/
	...
	new error
	switch(callfunc_begin("start_vote", "multimod_edit.amxx"))
	{
		case -1:
		{
