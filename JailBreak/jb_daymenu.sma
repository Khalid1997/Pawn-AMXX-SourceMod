...
new const g_szDaymenuObjectives[MAX_DAYS][] = 
{
	...
	"Kill the opposite team with HE-Grenades.",
...
{	
	...
	g_iFreeDayCheckForward = CreateMultiForward("is_freeday", ET_CONTINUE)
	
	if(g_iFreeDayCheckForward < 0)
	{
		set_fail_state("Forward can't be created 'is_freeday'")
	}
...
public Cmd_DaysMenu( id )
{
	...
	new iRet
	ExecuteForward(g_iFreeDayCheckForward, iRet)
	
	if(iRet)
	{
		ColorChat( id, NORMAL, "%s It's the free day time!! Wait till next round", g_szPrefix );
		return PLUGIN_HANDLED
	}
	...
	switch( g_iCurrentDay )
	{
		...
		{	
			...
			for( new i = 0; i < iNum; i++ )
			{
				...
				//give_item(iPlayers[i], "weapon_hegrenade")
			}
			
			set_task( 1.0, "startDodgeballDay", TASK_DODGEBALL );
			//startDodgeballDay()
		}
