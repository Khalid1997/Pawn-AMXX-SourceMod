...
// Levels and XP Things
new PlayersLevels[33];
new PlayersXP[33];
...
public plugin_init( )
{
	...
	register_concmd("amx_add_xp", "GiveXP", ADMIN_KICK, "<name | @all> <XP>" );
	register_concmd("amx_give_points", "GivePointsCmd", ADMIN_KICK, "<name | @all> <points>" );
	register_concmd("amx_set_points", "SetPointsCmd", ADMIN_KICK, "<name | @all> <points>" );
	register_concmd("amx_take_points", "TakePointsCmd", ADMIN_KICK, "<name | @all> <points>" );
	...
	register_clcmd( "say points", "PointsCmd" );
	register_clcmd( "say /level", "LevelXpCmd" );
	register_clcmd( "say level", "LevelXpCmd" );
	register_clcmd( "say /xp", "LevelXpCmd" );
	register_clcmd( "say xp", "LevelXpCmd" );
...
public GiveXP( id, level, cid )
{
	...
	{
		addxp = str_to_num( szArg2 );
	}
	else
	{
		client_print( id, print_console, "%s Sorry, Invalid Number.", g_szPrefix[2] );
		return PLUGIN_HANDLED;
	}
	...
	if( szArg1[0] == '@' )
	{
		...
		{
			AddNewXP( Players[i], addxp );
		}
		
		if(iPNum)
		{
			ColorPrint( 0, "ADMIN ^3%s: ^1gave ^3Everyone's ^1xp to ^3%d^1.", szAdminName, addxp );
		}
	...
	{
		new iTarget = cmd_target( id, szArg1, (CMDTARGET_ALLOW_SELF|CMDTARGET_NO_BOTS) );
		...
		AddNewXP( iTarget, addxp );
		get_user_name( iTarget, szPlayerName, charsmax( szPlayerName ) );
		ColorPrint( 0, "ADMIN ^3%s: ^1gave ^3%d ^1xp to ^3%s^1.", szAdminName, addxp, szPlayerName );
	}
...
public GivePointsCmd( id, level, cid )
{
	if(!cmd_access( id, level, cid, 3 ) )
		return PLUGIN_HANDLED;
	...
	if( szArg1[0] == '@' )
	{
		...
		for( new i = 0 ; i < iPNum ; i++ )
		{
			set_user_points( Players[i], get_user_points(Players[i]) + newpoints );
		}
		
		if(iPNum)
		{
			ColorPrint( 0, "ADMIN ^3%s: ^1gave ^3Everyone's ^1points to ^3%d^1.", szAdminName, newpoints );
		}
	...
	else
	{
		...
		set_user_points( iTarget, get_user_points(iTarget) + newpoints );
		get_user_name( iTarget, szPlayerName, charsmax( szPlayerName ) );
		ColorPrint( 0, "ADMIN ^3%s: ^1gave ^3%d ^1points to ^3%s^1.", szAdminName, newpoints, szPlayerName );
	}
	
	return PLUGIN_HANDLED;
}

public TakePointsCmd( id, level, cid )
{
	if(!cmd_access( id, level, cid, 3 ) )
		return PLUGIN_HANDLED;
	...
	if( szArg1[0] == '@' )
	{
		...
		if(iPNum)
		{
			ColorPrint( 0, "ADMIN ^3%s: ^1took from ^3Everyone's ^1points to ^3%d^1.", szAdminName, newpoints );
		}
	...
	else
	{
		...
		set_user_points( iTarget, get_user_points(iTarget) - newpoints );
		get_user_name( iTarget, szPlayerName, charsmax( szPlayerName ) );
		ColorPrint( 0, "ADMIN ^3%s: ^1took ^3%d ^1points from ^3%s^1.", szAdminName, newpoints, szPlayerName );
	}
	
	return PLUGIN_HANDLED;
}

public LevelXpCmd( id )
{
	ColorPrint( id, "^1Your Level: ^3%d^1/^3%d ^1| Your XP: ^3%d^1/^3%d^1.", PlayersLevels[id] + 1, sizeof( LevelsXpNum ), PlayersXP[id], LevelsXpNum[PlayersLevels[id]] );
	return PLUGIN_CONTINUE;
}
...
public fw_PlayerTouchGun(iEnt, iPlayer)
{
	...
	if(!g_bWeaponFound && g_iEnt == iEnt)
	{
		...
		AddNewXP( iPlayer, XP_TAKE_WEAPON );
...
stock CreateGameMenus()
{
	...
	for( new i = 0 ; i < sizeof(g_iPointsMenuItems) ; i++ )
	{
		menu_additem( g_iPointsMenu, g_iPointsMenuItems[i] );
		
		if( !( i + 1 >= sizeof(g_iPointsMenuItems) ) )
...
stock AddNewXP( id, NewXP )
{
	PlayersXP[id] += NewXP;
	
	if( PlayersXP[id] >= LevelsXpNum[PlayersLevels[id]] )
	{
		come_here:
		PlayersXP[id] -= LevelsXpNum[PlayersLevels[id]];
		
		if( !( PlayersLevels[id] + 1 >= sizeof( LevelsXpNum ) ) )
			PlayersLevels[id]++;
...
GotNewLevel( id )
{
	switch( PlayersLevels[id] )
	{
		case XP_LEVEL_ONE:
		{
			// All Players begin in Level 1 so they will never get Level 1 again.
		}
		
		case XP_LEVEL_TWO:
		{
			set_user_points( id, get_user_points(id) + 10 );
			ColorPrint( id, "^4You got 10 points for level up." );
		}
		
		case XP_LEVEL_THREE:
		{
			set_user_points( id, get_user_points(id) + 20 );
			ColorPrint( id, "^4You got 20 points for level up." );
		}
		
		case XP_LEVEL_FOUR:
		{
			set_user_points( id, get_user_points(id) + 30 );
			ColorPrint( id, "^4You got 30 points for level up." );
		}
		
		case XP_LEVEL_FIVE:
		{
			set_user_points( id, get_user_points(id) + 40 );
			ColorPrint( id, "^4You got 40 points for level up." );
		}
		
		case XP_LEVEL_SIX:
		{
			set_user_points( id, get_user_points(id) + 60 );
			ColorPrint( id, "^4You got 60 points for level up." );
		}
		
		case XP_LEVEL_SEVEN:
		{
			set_user_points( id, get_user_points(id) + 70 );
			ColorPrint( id, "^4You got 70 points for level up." );
		}
		
		case XP_LEVEL_EIGHT:
		{
			set_user_points( id, get_user_points(id) + 80 );
			ColorPrint( id, "^4You got 80 points for level up." );
		}
		
		case XP_LEVEL_NINE:
		{
			set_user_points( id, get_user_points(id) + 90 );
			ColorPrint( id, "^4You got 90 points for level up." );
		}
		
		case XP_LEVEL_TEN:
		{
			set_user_points( id, get_user_points(id) + 100 );
			ColorPrint( id, "^4You got 100 points for level up." );
		}
		
		case XP_LEVEL_ELEVEN:
		{
			set_user_points( id, get_user_points(id) + 250 );
			ColorPrint( id, "^4You got 250 points for last level." );
		}
	}
}
