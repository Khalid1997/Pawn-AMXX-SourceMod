...
public AdminMenuHandler(id, menu, item)
{
	...
	switch(item)
	{
		case AM_POINTS_MENU:
		{
			...
			if(!g_iGivePointsMenu)
			{
				g_iGivePointsMenu = menu_create("Set Points Menu", "SetPointsNumMenuHandler");
				new szItemName[30];
...
public GiveShopMenuHandler(id, menu, item)
{
	...
	new menu = CreateMenu(id, "Give Shop Item to Player", "GiveShopNameMenuHandler");
...
public GiveShopNameMenuHandler(id, menu, item)
{
	DestroyMenu(id);
...
public SetPointsNumMenuHandler(id, menu, item)
{
	...
	if( g_iGiveThingNumber[id] < 0 )
	{
		...
		ColorPrint(id, "^1Invalid Number noob");
		return;
	}
			
	new menu = CreateMenu(id, "Set Points Menu", "SetPointsMenuHandler");
			
	new Players[32], PlayerName[32], PNum;
	get_players( Players, PNum, "c" );
			
	menu_additem( g_iGivePointsMenu, "Everyone" );
			
	for( new i = 0/*, szId[5]*/ ; i < PNum ; i++ )
	{
		get_user_name(Players[i], PlayerName, charsmax( PlayerName ) );
...
public SetPointsMenuHandler(id, menu, item)
{
	DestroyMenu(id);
	if(item == MENU_EXIT)
	{
	...
	switch(item)
	{
		case 0:
		{
			...
			for( new i = 0 ; i < PNum ; i++ )
			{
				set_user_points( Players[i], iGivePointsNum );
			}
		...
		default:
		{
			...
			set_user_points( PlayerId, iGivePointsNum );
...
public RemoveMenuHandler(id, menu, item)
{
	...
	if(item)
	{
		...
		if(GetHitAimStuff(id, iEnt, vDump) && is_valid_ent(iEnt))
		{
			...
			if(equal(szClassName, g_szTestSpawnEnt))
			{
				// there is a bug here idk maybe you must put same as
				// remove nearest entity :P
				iEntoto = pev(iEnt, pev_array_index);
				remove_entity(iEnt);
				ArrayDeleteItem(g_hArraySpawnOrigins, iEntoto);
				g_iWeapSpawnOrigins--;
