...
{
	g_hWeaponTouchForward = CreateMultiForward("CM_WeaponChoosed", ET_IGNORE, FP_CELL, FP_CELL)
	...
	forward CM_WeaponChoosed(id, iWeaponIndex)
...
{
	ArrayPushArray(g_hWeaponsArray, szMenuName);
	g_iWeaponsCount++;
...
{
	new Array[SHOP_ITEM_DATA];
	
	copy(Array[SHOP_ITEM_DATA], charsmax(Array[SHOP_ITEM_DATA]), szItemName);
	Array[SHOP_ITEM_PRICE] = iItemPrice;
	
	ArrayPushArray(g_hShopItemsArray, Array);
	++g_iShopItemsCount;
...
public GiveCustomPointsCmd(id, level, cid)
{
	...
	for( new i = 0/*, szId[5]*/, szName[32], szItemName[50] ; i < iNum ; i++ )
	{
		get_user_name(iPlayers[i], szName, charsmax( szName ) );
		//num_to_str(iPlayers[i], szId, charsmax(szId))
		
		formatex(szItemName, charsmax(szItemName), "%s \y(\r%d\y)", szName, g_iPoints[iPlayers[i]]);
...
#endif
{
	...
	//flMaxSpeed = entity_get_float(id, EV_FL_maxspeed);
...
{	
	return ( get_user_points(id) >= g_szShopItems[item][SHOP_ITEM_PRICE] ) ? ITEM_ENABLED : ITEM_DISABLED;
}
...
GivePointsMenuHandler(id, menu, item)
{
	...
	switch(item)
	{
		...
		default:
		{
			...
			//if(!PlayerId)
			{
				server_print("Not found %d", PlayerId);
				ColorPrint(id, "Player wasn't found.");
				return;
			}
			...
			ColorPrint( 0, "ADMIN ^3%s ^1Gave ^3%d ^1points to ^3%s^1.", AdminName, szPlayerName, iGivePointsNum );
		}
	}
}
