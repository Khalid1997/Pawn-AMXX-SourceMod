...
public plugin_init()
{
	...
	g_pHUD = register_cvar("amx_multimod_show_hud", "1")
...
Build()
{
	...
	{	
		...
		if(i == g_currentmodid)
		{
			...
			menu_additem(gMenu, szItem, szInfo, .callback = menu_makecallback("callback"))
		}
...
public show_hud()
{
	...
	for(new i; i < iCount; i++)
	{
		...
		show_hudmessage(iPlayers[i], "Current Mod: %s^nNext mod: %s", g_modnames[g_currentmodid], g_nextmodid ? g_modnames[g_nextmodid] : "Not choosen yet.")
	}
	...
	if(get_pcvar_num(g_pRTMV))
	{
		...
		else client_print(0, print_chat, "%s Type 'rtmv' or 'rtv' to rock the mod vote!", PREFIX)
		#endif
	}
