...
#define UNLIMITED_MODS
	...
	//#define SHOW_PERCENT_AFTER_CHOOSING
...
new gRecentMods
new g_szRecentModsFile[60]
new Trie:g_TrieRecentMods
...
public plugin_init()
{
	...
	register_clcmd("say /mm_menu", "AdminCmdMM_Menu", ADMIN_RCON)
...
public GetRecentMaps()
{
	...
	new szLine[70], szModPrefix[SSTRING], szTimeStamp[SSTRING], iModNum
	while(!feof(f))
	{
		...
		if( ( iTimeStamp = str_to_num(szTimeStamp) ) < iSysTime )
		{
			if( ( iModNum = ModExists(szModPrefix) ) == -1) 
			{
				continue;
			}
			
			ArrayPushCell(g_ArrayRecentMods, 
		}
	}
...
{	
	...
	if( g_votemodcount[b] <= ( iTotalVotes / 2 ) )
	{
...
public check_votes_runoff(iParams[])
{
	...
	if(g_votemodcount[b] == g_votemodcount[c])
	{
		...
		set_hudmessage(0, 255, 0, -1.0, 0.20, 0, 0.0, 5.0, 0.1, 0.1, -1)
		ShowSyncHudMsg(0, gSyncHud, "The two mods got the same votes!^nChoosing a random mod!^n^nThe NEXT MOD WILL BE %s^nThe map vote will start in 5 seconds!", g_modnames[iWinningOption])
